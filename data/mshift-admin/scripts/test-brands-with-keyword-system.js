...
async function testSingleBrand(brand) {
  try {
    ...
    performanceMetrics.minProcessingTime = Math.min(performanceMetrics.minProcessingTime, processingTime);
  ...
  for (let offset = 0; offset < totalBrands; offset += batchSize) {
    ...
    for (const brand of brands) {
      ...
      if (processed % 50 === 0) {
        ...
        const avgTime = Math.round(performanceMetrics.totalProcessingTime / stats.total);
        console.log(`  ✓ ${processed}/${totalBrands} 완료 - 정확도: ${currentAccuracy}% - 평균 응답: ${avgTime}ms`);
      }
    ...
    console.log(`✅ 배치 완료 - 진행률: ${progress}% - 정확도: ${currentAccuracy}%`);
  }
  
  // 평균 처리 시간 계산
  performanceMetrics.avgProcessingTime = Math.round(performanceMetrics.totalProcessingTime / stats.total);
...
function analyzeSystemPerformance() {
  ...
  const lowPerformanceTags = Object.entries(stats.byTag)
    .filter(([tag, data]) => (data.success / data.total) < 0.6 && data.total >= 10)
...
function generateKeywordPatterns() {
  ...
  for (const [tag, items] of brandsByTag.entries()) {
    if (items.length >= 5) { // 5개 이상의 실패 케이스가 있는 태그만
      ...
      if (commonKeywords.length > 0) {
        ...
        console.log(`  📝 ${tag} 태그 새 키워드 제안: [${commonKeywords.join(', ')}] (${items.length}개 실패 케이스)`);
      }
    }
  }
...
## 🛣️ 처리 경로별 분석
${Object.entries(stats.byProcessingPath).map(([path, count]) => 
...
`;
  ...
  const reportPath = path.join(__dirname, '../test-results/keyword-system-test-report.md');
...
async function main() {
  try {
    ...
    console.log('📚 시스템 설계 기준: 77% 정확도 (1,051개 키워드, 32개 태그)\n');
    ...
    if (finalReport.accuracy < 77) {
      ...
      console.log(`📋 이 패턴들을 keyword_groups 테이블에 추가하여 정확도를 개선할 수 있습니다.`);
    } else {
