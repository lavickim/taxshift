...
async function migrateData() {
  try {
    ...
    if (gyeonggiData.rows.length > 0) {
      ...
      for (let i = 0; i < gyeonggiData.rows.length; i++) {
        ...
        try {
          ...
          if (i % 1000 === 0) {
            console.log(`📦 경기도 배달특급 데이터 ${i + 1}/${gyeonggiData.rows.length} 완료`)
          }
        } catch (error) {
          console.log(`⚠️ 경기도 데이터 삽입 오류 (계속 진행): ${error.message}`)
        }
      }
    }
    ...
    const seoulData = await sourceClient.query('SELECT * FROM datacollection_seoul_restaurants ORDER BY created_at LIMIT 5000')
    console.log(`🍽️ 서울시 음식점 데이터: ${seoulData.rows.length}개 (처음 5,000개만)`)
    ...
    if (seoulData.rows.length > 0) {
      for (let i = 0; i < seoulData.rows.length; i++) {
        ...
        try {
          ...
          if (i % 500 === 0) {
            console.log(`🍽️ 서울시 음식점 데이터 ${i + 1}/${seoulData.rows.length} 완료`)
          }
        } catch (error) {
