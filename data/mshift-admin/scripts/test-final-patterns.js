...
function runFinalTest() {
  ...
  if (successRate >= 75) {
    ...
    console.log(`⚠️  목표 미달성. ${successRate}%는 75% 목표에 ${75 - successRate}%p 부족합니다.`);
  }
  ...
  const finalResults = {
    ...
    mvpReadiness: {
      ...
      rating: successRate >= 80 ? '우수' : 
              successRate >= 70 ? '양호' : 
              successRate >= 60 ? '보통' : '부족'
    },
    recommendations: []
  };

  if (successRate < 75) {
    finalResults.recommendations.push('75% 목표 달성을 위해 추가 키워드 패턴이 필요합니다.');
  }
  ...
  if (unmatchedCases.length > 50) {
    ...
    if (commonWords.length > 0) {
      finalResults.recommendations.push(`자주 나타나는 미매칭 패턴: ${commonWords.join(', ')}`);
    }
  }
  ...
  fs.writeFileSync(path.join(outputDir, 'final-test-results.json'), JSON.stringify(finalResults, null, 2));
