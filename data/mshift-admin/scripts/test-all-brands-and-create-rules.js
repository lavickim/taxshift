...
async function testSingleBrand(brand) {
  try {
    const response = await axios.post(CLASSIFICATION_API, {
    ...
    const matched = result.matched && result.tag && expectedTags.includes(result.tag);
  ...
  for (let offset = 0; offset < totalBrands; offset += batchSize) {
    ...
    for (const brand of brands) {
      ...
      if (processed % 100 === 0) {
        ...
        console.log(`  ✓ ${processed}/${totalBrands} 테스트 완료 - 현재 정확도: ${currentAccuracy}%`);
      }
...
function generateRulePatternsFromFailures() {
  ...
  for (const [tag, brands] of brandPatterns.entries()) {
    if (brands.size >= 3) { // 3개 이상의 브랜드가 있는 태그만 룰 생성
  ...
  for (const [conflict, cases] of conflictPatterns.entries()) {
    if (cases.length >= 2) {
      ...
      const brands = cases.map(c => c.brand.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
      ...
      console.log(`  🔄 충돌 해결 룰: ${expectedTag} vs ${actualTag} (${cases.length}개)`);
    }
  }
...
async function generateDetailedReport() {
  ...
  const lowPerformanceTags = Object.entries(stats.byTag)
    .filter(([tag, data]) => (data.success / data.total) < 0.8 && data.total >= 10)
...
async function main() {
  try {
    ...
    if (report.totalAccuracy < 95) {
      ...
      console.log(`💡 현재 정확도: ${report.totalAccuracy.toFixed(2)}% (목표: 95%)`);
