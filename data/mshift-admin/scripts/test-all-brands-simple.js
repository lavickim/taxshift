...
const simpleRules = [
  // 치킨 관련
  { keywords: ['치킨', '닭', 'chicken', '통닭', '후라이드', '양념'], tag: '치킨' },
  
  // 카페 관련
  { keywords: ['카페', 'cafe', '커피', 'coffee', '스타벅스', '투썸', '이디야', '빈', '로스터리'], tag: '카페' },
  
  // 편의점 관련
  { keywords: ['편의점', '세븐일레븐', '7-eleven', 'cu', '씨유', 'gs25', '이마트24', '위드미'], tag: '편의점' },
  
  // 피자 관련
  { keywords: ['피자', 'pizza', '도미노', '피자헛', '파파존스', '미스터피자'], tag: '피자' },
  
  // 교육 관련
  { keywords: ['학원', '교육', '스쿨', 'school', '어학원', '태권도', '수학', '영어', '유치원'], tag: '교육' },
  
  // 스포츠 관련
  { keywords: ['헬스', 'gym', '피트니스', '요가', '필라테스', '골프', '테니스', '수영'], tag: '스포츠' },
  
  // 뷰티 관련
  { keywords: ['뷰티', '미용', '네일', '헤어', '살롱', '에스테틱', '마사지'], tag: '뷰티' },
  
  // 패스트푸드 관련
  { keywords: ['버거', 'burger', '맥도날드', '롯데리아', 'kfc', '서브웨이', '햄버거'], tag: '패스트푸드' },
  
  // 디저트 관련
  { keywords: ['아이스크림', '빙수', '디저트', '케이크', '도넛', '젤라또'], tag: '디저트' },
  
  // 베이커리 관련
  { keywords: ['베이커리', '빵', '파리바게뜨', '뚜레쥬르', '제과점'], tag: '베이커리' }
];
...
async function testAllBrands() {
  ...
  for (let offset = 0; offset < totalBrands; offset += batchSize) {
    ...
    for (const brand of brands) {
      const result = classifyBrand(brand.brandName, brand.generatedTransactionString);
    ...
    console.log(`✅ 배치 완료 - 진행률: ${((processed / totalBrands) * 100).toFixed(1)}% - 정확도: ${currentAccuracy}%`);
    
    if (processed % 1000 === 0) {
      console.log(`📈 현재까지 정확도: ${currentAccuracy}% (${stats.success}/${stats.total})`);
    }
  }
  ...
  console.log(`🎯 정확도: ${((stats.success / stats.total) * 100).toFixed(2)}%`);
  ...
  for (const [tag, brands] of brandsByTag.entries()) {
    if (brands.length >= 3) {
      ...
      if (keywords.length > 0) {
        ...
        console.log(`  📝 ${tag} 태그 룰 생성: ${keywords.join(', ')} (${uniqueBrands.length}개 브랜드)`);
      }
    }
  }
...
function generateDetailedReport(results) {
  ...
  const lowPerformanceTags = Object.entries(results.stats.byTag)
    .filter(([tag, data]) => (data.success / data.total) < 0.7 && data.total >= 10)
...
async function main() {
  try {
    ...
    if (results.totalAccuracy < 95) {
      ...
      console.log(`🔧 이 룰들을 시스템에 추가하면 정확도를 개선할 수 있습니다.`);
      
      // 새 룰 출력
      if (newRules.length > 0) {
        console.log('\n📝 생성된 룰 패턴:');
        newRules.forEach((rule, index) => {
          console.log(`  ${index + 1}. ${rule.tag}: [${rule.keywords.join(', ')}] (${rule.brands.length}개 브랜드)`);
        });
      }
    } else {
