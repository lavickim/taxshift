...
export async function GET() {
  try {
    ...
    const testedBrands = await prisma.franchiseBrands.count({
      where: { testPassed: { not: null } }
    });
    const passedBrands = await prisma.franchiseBrands.count({
      where: { testPassed: true }
    });
    const failedBrands = await prisma.franchiseBrands.count({
      where: { testPassed: false }
    });

    // 상위 태그 통계
    const topTags = await prisma.franchiseBrands.groupBy({
      by: ['primaryTag'],
      _count: {
        primaryTag: true
      },
    ...
    // 산업분류별 통계
    const industryStats = await prisma.franchiseBrands.groupBy({
    ...
    const recentTests = await prisma.franchiseBrands.findMany({
      where: {
        lastTestAt: {
          gte: sevenDaysAgo
        }
