...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('GET /api/v2/tag-mapping-mgmt/keyword-tag-mappings - 키워드-태그 매핑 목록 조회');
    ...
    console.log('키워드-태그 매핑 조회 성공:', Array.isArray(data) ? data.length : 'Non-array data');
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${JAVA_API_BASE_URL}/v2/tag-mapping-mgmt/keyword-tag-mappings`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
