# v1.0 MoneyShift 룰엔진 시스템 종합 기술 설계서
...
### 시스템 목적
MoneyShift 룰엔진 시스템은 **정규식 기반 전처리 시스템**을 핵심으로 하는 **향상된 4-Layer 처리 파이프라인**으로, 복잡한 한국 거래 문자열을 정확하게 정규화하고 분류하여 높은 정확도와 성능을 동시에 달성합니다.

### v1.0 핵심 특징
- **정규식 전처리 레이어**: 거래 문자열 정규화 → 키워드 매칭 파이프라인
- **지능형 패턴 매칭**: 법인구조, 주유소, 해외서비스 등 특화 정규식 패턴
- **어드민 통합 관리**: LLM 지원 패턴 관리 및 실시간 테스트 시스템
- **동적 학습**: 사용자 피드백 기반 지속적인 성능 향상
- **비용 최적화**: 전처리 → 키워드 매칭 → LLM 순차 적용으로 비용 최소화
...
```mermaid
graph TD
    A[거래 원문] --> B[Layer 0: Redis Cache]
    B --> |Cache Miss| C[정규식 전처리 시스템]
    C --> D[Layer 1: Keyword Engine]
    D --> |매칭 실패| E[Layer 2: ML Inference]
    E --> |신뢰도 부족| F[Layer 3: LLM Fallback]
...
### 정규식 전처리 시스템 상세
```
거래 원문: "(주)코드쉬프트 PAYMENT SAN FRANCISCO USA -50000"
     ↓
[정규식 전처리 시스템]
├── 법인구조 정규화: "(주)코드쉬프트" → "코드쉬프트"
├── 해외서비스 정규화: "PAYMENT SAN FRANCISCO USA" → "결제서비스"
├── 금액 제거: "-50000" → ""
└── 최종 정규화: "코드쉬프트 결제서비스"
     ↓
[Layer 1: Keyword Engine] → 키워드 매칭 → 태그/계정과목
```
...
처리 방식:
  ...
  2. 출력 템플릿 적용하여 정규화
  3. 메타데이터 추출 (업종, 위치 등)
  4. 정규화된 텍스트 → Keyword Engine 전달

패턴 카테고리:
  - 법인구조: "(주)코드쉬프트" → "코드쉬프트"
  - 주유소: "GS칼텍스셀프 강남(상)주" → "GS칼텍스 강남 상행선 주유소"
  - 해외서비스: "CLAUDE.AI SUBSCRIPTION" → "Claude AI"
  - 대형마트: "이마트 에브리데이 서" → "이마트"
  - 정부기관: "국민연금관리공단" → "국민연금"
...
```yaml
목적: 정규화된 텍스트의 패턴 기반 고속 매칭
구현: KeywordExtractionEngine (전처리 시스템 연동)
정확도: 90%+ (정규식 전처리 후)

처리 방식:
  1. 정규식 전처리 시스템에서 정규화된 텍스트 수신
