...
# Trojan Horse 백엔드 실행 스크립트
# Spring Boot 서버를 포트 8081에서 실행

echo "🚀 Trojan Horse 백엔드 시작 중..."

# 현재 디렉토리 확인
if [[ ! -d "mshift-trojan-backend" ]]; then
    echo "❌ 오류: mshift-trojan-backend 디렉토리를 찾을 수 없습니다."
...
# Spring Boot 프로세스 확인 및 종료 (TrojanApplication 기준)
echo "🔍 기존 Trojan 백엔드 프로세스 확인 중..."
TROJAN_PID=$(ps aux | grep TrojanApplication | grep -v grep | awk '{print $2}')
if [[ ! -z "$TROJAN_PID" ]]; then
    echo "⏹️  기존 Trojan 백엔드 프로세스 종료 중... (PID: $TROJAN_PID)"
    kill -9 $TROJAN_PID
    sleep 3
fi
...
echo "🏗️  Trojan 백엔드 빌드 및 실행 중..."
cd mshift-trojan-backend

# 백그라운드에서 실행하고 PID 저장
nohup mvn spring-boot:run > ../backend-trojan.log 2>&1 &
BACKEND_PID=$!

echo "📝 백엔드 PID: $BACKEND_PID (backend-trojan.log에 로그 저장됨)"

# 서버 시작 대기
echo "⏳ 서버 시작 대기 중..."
for i in {1..30}; do
    if curl -s http://localhost:8081/api/actuator/health > /dev/null 2>&1; then
        echo "✅ Trojan 백엔드가 성공적으로 시작되었습니다!"
        ...
        echo ""
        echo "🎯 로그 확인: tail -f backend-trojan.log"
        exit 0
    fi
    sleep 2
    echo -n "."
...
echo "   tail -f backend-trojan.log"
exit 1