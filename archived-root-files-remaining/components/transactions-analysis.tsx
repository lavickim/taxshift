...
export function TransactionsAnalysis() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (data.success) {
        setNormalizedResults(JSON.stringify(data.normalization_results, null, 2));
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6 bg-card border rounded-lg shadow-sm">
      <h2 className="text-2xl font-bold mb-6 text-center">거래 내역 분석 (룰베이스 구축)</h2>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full h-32 p-3 border border-border rounded-md resize-none focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent font-mono text-sm"
            />
            <p className="text-xs text-muted-foreground mt-1">
              총 {transactions.split('\n').filter(line => line.trim().length > 0).length}개 거래 내역
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="normalized-results" className="block text-sm font-medium mb-2">
              ...
              <textarea
                ...
                className="w-full h-80 p-3 border border-border rounded-md resize-none bg-blue-50/50 focus:outline-none font-mono text-xs"
              />
            </div>
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="analysis-results" className="block text-sm font-medium mb-2">
              ...
              <textarea
                ...
                className="w-full h-80 p-3 border border-border rounded-md resize-none bg-green-50/50 focus:outline-none font-mono text-xs"
              />
            </div>
