...
export function AdminTestDashboard() {
  ...
  const [dbInfo, setDbInfo] = useState<DatabaseInfo>({ connected: false, tables: [], rowCounts: {} });
  const [testInputs, setTestInputs] = useState({
    regexText: "GS25강남역점",
    llmText: "구글페이먼트코리아",
    cacheHash: "a1b2c3d4e5f67890123456789012345678901234567890123456789012345678",
  ...
  const StatusIcon = ({ status }: { status: string }) => {
    switch (status) {
      case 'loading': return <span className="text-blue-500">🔄</span>;
    ...
    <div className="w-full space-y-6">
      ...
      <Alert className="border-green-200 bg-green-50/50">
        <div className="flex items-center">
          ...
          <div>
            <AlertTitle className="text-green-800">관리자 테스트 대시보드</AlertTitle>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Layer 0: Cache</CardTitle>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Layer 1: Regex</CardTitle>
                <span className="text-2xl">📄</span>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Layer 2: ML</CardTitle>
                <span className="text-2xl">📊</span>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Layer 3: LLM</CardTitle>
                <span className="text-2xl">🧠</span>
              </CardHeader>
              <CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle>🚀 MoneyShift AI Tax Service - 4-Layer Defense Architecture</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="text-sm text-foreground font-medium">
                한국 AI 세무 서비스의 4단계 방어 아키텍처 (95% Rules + 4% ML + 1% LLM)
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
                  <span className="text-2xl">🔍</span>
                  <div>
                    <div className="font-medium text-blue-900 dark:text-blue-100">Layer 0: Cache System</div>
                ...
                <div className="flex items-center gap-3 p-3 bg-green-50 dark:bg-green-950/20 rounded-lg">
                  <span className="text-2xl">📄</span>
                  <div>
                    <div className="font-medium text-green-900 dark:text-green-100">Layer 1: Regex Engine (95%)</div>
                ...
                <div className="flex items-center gap-3 p-3 bg-orange-50 dark:bg-orange-950/20 rounded-lg">
                  <span className="text-2xl">📊</span>
                  <div>
                    <div className="font-medium text-orange-900 dark:text-orange-100">Layer 2: ML Engine (4%)</div>
                ...
                <div className="flex items-center gap-3 p-3 bg-purple-50 dark:bg-purple-950/20 rounded-lg">
                  <span className="text-2xl">🧠</span>
                  <div>
                    <div className="font-medium text-purple-900 dark:text-purple-100">Layer 3: LLM Engine (1%)</div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Button 
                  ...
                  className="w-full"
                  disabled={testResults['GET-/api/test-db-connection']?.status === 'loading'}
                >
                  ▶️ DB 연결 테스트
                </Button>
                ...
                {testResults['GET-/api/test-db-connection'] && (
                  <div className="mt-4 p-3 bg-muted rounded-lg">
                    <pre className="text-xs">
                      {JSON.stringify(testResults['GET-/api/test-db-connection'], null, 2)}
                    </pre>
                  </div>
                )}
                    ...
                    {dbInfo.tables.map((table) => (
                      <div key={table} className="flex items-center justify-between p-3 bg-muted/50 rounded">
                        ...
                        <Badge variant="outline">
                          {dbInfo.rowCounts[table] !== undefined ? `${dbInfo.rowCounts[table]} rows` : 'Unknown'}
                        </Badge>
                      </div>
                    ))}
                  </div>
        ...
        <TabsContent value="endpoints" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                {[
                  { endpoint: '/api/test-db-connection', method: 'GET', label: 'DB 연결 테스트' },
                  { endpoint: '/api/cache/stats', method: 'GET', label: '캐시 통계' },
                  { endpoint: '/api/cache/lookup', method: 'GET', label: '캐시 조회' },
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setTestInputs(prev => ({ ...prev, regexText: e.target.value }))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Button
                  onClick={() => testApi('/api/regex/match', 'POST', { text: testInputs.regexText })}
                  disabled={testResults['POST-/api/regex/match']?.status === 'loading'}
                >
                  📄 정규식 매칭 테스트
                </Button>

                <Button
                  onClick={() => testApi('/api/llm/infer', 'POST', { text: testInputs.regexText })}
                  disabled={testResults['POST-/api/llm/infer']?.status === 'loading'}
                >
                  🧠 LLM 분류 테스트
                </Button>
              </div>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => setTestInputs(prev => ({ ...prev, analysisText: e.target.value }))}
