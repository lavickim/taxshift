...
export function IncheonRestaurantsAnalysis() {
  ...
  const fetchData = async (type: string, page: number = 1) => {
    try {
      ...
      const response = await fetch(`/api/data-analysis/incheon-restaurants?${params}`)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-3xl font-bold tracking-tight text-green-600">인천 음식점 분석</h2>
        <Badge variant="secondary" className="bg-green-100 text-green-800">
          {data.overview ? formatNumber(data.overview.totalCount) : '0'}개 음식점
        </Badge>
      </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">총 음식점 수</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-blue-600">
                  {data.overview ? formatNumber(data.overview.activeCount) : '0'}
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">지역 커버리지</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={400}>
                <BarChart data={data.region || []}>
                  ...
                  <YAxis />
                  <Tooltip formatter={(value) => [formatNumber(Number(value)), '음식점 수']} />
            ...
            <CardContent>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      <th className="text-left p-2">음식점명</th>
                      <th className="text-left p-2">영업상태</th>
                  ...
                  <tbody>
                    {(data.details?.items || []).map((item: any, index) => (
                      <tr key={index} className="border-b hover:bg-gray-50">
                        ...
                        <td className="p-2">
                          <Badge variant={item.business_status === '영업' ? "default" : "secondary"}>
                            {item.business_status}
                          </Badge>
                        </td>
                        <td className="p-2">{item.district}</td>
                        <td className="p-2 max-w-xs truncate">{item.address}</td>
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-gray-500">
                  ...
                  {((currentPage - 1) * 50) + 1}-{Math.min(currentPage * 50, data.details?.total || 0)}개 표시
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => setCurrentPage(p => p + 1)}
                    disabled={currentPage >= Math.ceil((data.details?.total || 0) / 50)}
                  >
                    다음
                  </Button>
                </div>
