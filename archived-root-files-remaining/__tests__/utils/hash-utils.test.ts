...
describe('hash-utils', () => {
  describe('generateHash', () => {
    ...
    it('빈 문자열을 처리할 수 있어야 함', () => {
      ...
      expect(result).toHaveLength(64);
      expect(result).toMatch(/^[a-f0-9]{64}$/);
      expect(result).toBe('e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855'); // 빈 문자열의 SHA256
    });

    it('null과 undefined를 적절히 처리해야 함', () => {
      expect(() => generateHash(null as any)).toThrow('Input text cannot be null or undefined');
      expect(() => generateHash(undefined as any)).toThrow('Input text cannot be null or undefined');
    });
    ...
    it('성능 요구사항을 만족해야 함 (1000건 < 50ms)', () => {
      const testTexts = Array.from({ length: 1000 }, (_, i) => `test string ${i}`);
  ...
  describe('normalizeText', () => {
    ...
    it('null과 undefined를 적절히 처리해야 함', () => {
      expect(() => normalizeText(null as any)).toThrow('Input text cannot be null or undefined');
      expect(() => normalizeText(undefined as any)).toThrow('Input text cannot be null or undefined');
    });
  ...
  describe('통합 테스트', () => {
    ...
    it('대량 거래 데이터 처리 성능 테스트', () => {
      const largeDataset = Array.from({ length: 5000 }, (_, i) => `거래 데이터 ${i}`);
