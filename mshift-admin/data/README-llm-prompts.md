...
이 파일(`data/llm-prompts.ts`)은 프로젝트에서 사용되는 모든 LLM 프롬프트를 중앙에서 관리하기 위해
만들어졌습니다.
...
const prompt = buildTransactionClassificationPrompt('지에스25 강남점', {
  amount: 5000,
  date: '2024-01-15',
});
...
- [`app/api/transactions-analysis/route.ts`](../app/api/transactions-analysis/route.ts) - 거래 분석
  API
