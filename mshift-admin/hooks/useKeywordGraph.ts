...
function createMockKeywordGraphData(): KeywordGraphResponse {
  ...
  return {
    keywords: mockNodes.map(node => ({
      id: node.id,
      keyword: node.id,
      category: node.category,
    })),
    relationships: mockLinks.map(link => ({
      source: link.source,
      target: link.target,
    ...
    statistics: {
      ...
      totalMembers: mockNodes.reduce(
        (sum, node) => sum + (node.memberCount || 0),
        0
      ),
      totalIndustries: mockNodes.reduce(
        (sum, node) => sum + (node.industryCount || 0),
        0
      ),
...
export function useKeywordGraph(
  options: UseKeywordGraphOptions = {}
): UseKeywordGraphReturn {
  const {
    autoLoad = false,
    minMemberCount = 100,
    maxIndustries = 150,
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          console.warn(
            '백엔드 서버에 연결할 수 없습니다. 실제 54만건 데이터 처리를 위해 서버 시작을 기다리는 중...'
          );
          setError(
            '🚀 54만건 국민연금 데이터 처리를 위해 백엔드 서버가 시작 중입니다. 잠시 후 다시 시도해주세요.'
          );
  ...
  const loadPreview = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/v2/optimized-keyword-graph/preview`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const loadCached = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/v2/optimized-keyword-graph/cached?maxKeywords=100&maxRelationships=200`
      );
      ...
      if (
        ...
        console.warn(
          '백엔드 서버에 연결할 수 없습니다. 서버 시작을 기다리는 중...'
        );
  ...
  useEffect(() => {
    const checkHealth = async () => {
      try {
        const response = await fetch(
          `${API_BASE}/v2/optimized-keyword-graph/health`
        );
        if (!response.ok) {
...
export function useKeywordGraphPerformance() {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/v2/optimized-keyword-graph/performance-stats`
      );
      if (response.ok) {
        const result = await response.json();
        setStats(result);
      ...
      if (
        ...
        console.warn(
          '백엔드 서버에 연결할 수 없습니다. Mock 성능 데이터를 사용합니다.'
        );
        setStats({
