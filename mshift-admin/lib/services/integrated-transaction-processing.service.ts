...
import { RegexPreprocessingEngine, ProcessingResult } from './regex-preprocessing.service';
...
export class IntegratedTransactionProcessingService {
  ...
  // Java 백엔드 URL
  private readonly JAVA_API_BASE_URL = process.env.JAVA_API_BASE_URL || 'http://localhost:8080';
  
  static getInstance(): IntegratedTransactionProcessingService {
    if (!IntegratedTransactionProcessingService.instance) {
      IntegratedTransactionProcessingService.instance = new IntegratedTransactionProcessingService();
    }
  ...
  async processTransaction(
    ...
    try {
      ...
      const preprocessingResult = await this.regexEngine.preprocess(originalText);
      ...
      if (result.keywordExtractionSuccess && keywordResult.tag) {
        ...
        result.finalAccount = keywordResult.accountName || keywordResult.accountCode;
  ...
  private async callKeywordExtractionAPI(
    ...
    try {
      const response = await fetch(`${this.JAVA_API_BASE_URL}/v2/keyword-system/classify`, {
        method: 'POST',
        headers: {
  ...
  private logPerformanceMetrics(result: IntegratedProcessingResult): void {
    const metrics = {
      ...
      preprocessingRatio: (result.preprocessingTime / result.totalProcessingTime) * 100,
      keywordRatio: (result.keywordExtractionTime / result.totalProcessingTime) * 100,
    ...
    if (result.finalConfidence < 0.7 && result.processingPath === 'FULL_PIPELINE') {
   ...
   * 통계 수집
   */
  async getProcessingStats(dateFrom?: Date, dateTo?: Date): Promise<ProcessingStats> {
