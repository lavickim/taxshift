...
export function JournalEntryManagement() {
  ...
  const loadJournalEntries = async () => {
    try {
      ...
      const response = await fetch(
        `/api/v2/accounting/journal-entries?${params}`
      );
  ...
  const createComplexJournalEntry = async () => {
    ...
    try {
      const response = await fetch(
        '/api/v2/accounting/journal-entries/complex',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const approveJournalEntry = async (entryId: number) => {
    ...
    try {
      const response = await fetch(
        `/api/v2/accounting/journal-entries/${entryId}/approve`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
        }
  ...
  const postJournalEntry = async (entryId: number) => {
    ...
    try {
      const response = await fetch(
        `/api/v2/accounting/journal-entries/${entryId}/post`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        }
  ...
  const showJournalEntryDetails = async (entry: JournalEntry) => {
    try {
      const response = await fetch(
        `/api/v2/accounting/journal-entries/${entry.id}`
      );
                ...
                <TableBody>
                  {journalEntries.map(entry => (
                    <TableRow key={entry.id}>
                      ...
                      <TableCell>
                        {formatCurrency(entry.totalDebitAmount)}
                      </TableCell>
        ...
        <TabsContent value='complex'>
          <Card>
            ...
            <CardContent className='space-y-4'>
              ...
              <Button
                onClick={createComplexJournalEntry}
                disabled={isLoading}
                className='w-full'
              >
                {isLoading ? (
