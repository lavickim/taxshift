...
import { Play, Plus, Trash2, Save, TestTube, AlertCircle, CheckCircle, Clock, Zap } from "lucide-react";
      ...
      for (const input of testInputs) {
        ...
        try {
          ...
          if (match) {
            ...
            const expectedOutput = rule.testCases?.find(tc => tc.input === input)?.expected;
            ...
            results.push({
              ...
              success: isSuccess,
              errorMessage: isSuccess ? undefined : `Expected "${expectedOutput}", got "${output}"`
            });
          } else {
            results.push({
              input,
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <Input
                type="number"
                value={rule.priority}
                onChange={(e) => setRule(prev => ({ ...prev, priority: parseInt(e.target.value) || 100 }))}
                placeholder="100"
              />
            </div>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Textarea
              ...
              onChange={(e) => setRule(prev => ({ ...prev, inputPattern: e.target.value }))}
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => setRule(prev => ({ ...prev, outputTemplate: e.target.value }))}
              placeholder="예: $1"
            />
                ...
                <div key={testCase.id} className="flex items-center justify-between p-3 border rounded">
                  <div className="flex-1">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <span className="text-sm text-gray-600">예상 출력:</span>
            ...
            <div className="space-y-4 mt-6">
              ...
              <div className="space-y-2">
                {testResults.map((result, index) => (
                  <div 
                    key={index}
                    className={`p-3 border rounded ${result.success ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}
                  >
                    ...
                    {result.success ? (
                      <div className="text-sm">
                        ...
                        <span className="font-mono ml-2 text-green-700">{result.output}</span>
                        {result.expected && result.output !== result.expected && (
                          <div className="mt-1">
                            <span className="text-gray-600">예상:</span>
                            <span className="font-mono ml-2 text-gray-700">{result.expected}</span>
                          </div>
                        )}
                      </div>
                    ) : (
