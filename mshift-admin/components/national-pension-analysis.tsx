...
export function NationalPensionAnalysis() {
  ...
  const [companyPagination, setCompanyPagination] =
    useState<CompanyPaginationData>({
      page: 1,
      limit: 100,
      total: 0,
      pages: 0,
    });
  const [companyFilters, setCompanyFilters] = useState<CompanyFiltersData>({
    search: '',
    region: '',
    industry: '',
    sortBy: 'employees',
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [
        ...
        fetch('/api/data-analysis/national-pension?type=overview').then(r =>
          r.json()
        ),
        fetch('/api/data-analysis/national-pension?type=status').then(r =>
          r.json()
        ),
        fetch('/api/data-analysis/national-pension?type=region').then(r =>
          r.json()
        ),
        fetch('/api/data-analysis/national-pension?type=industry').then(r =>
          r.json()
        ),
        fetch('/api/data-analysis/national-pension?type=company-size').then(r =>
          r.json()
        ),
        fetch('/api/data-analysis/national-pension?type=top-companies').then(
          r => r.json()
        ),
      ]);
      ...
      if (topCompaniesData.success)
        setTopCompanies(topCompaniesData.data.slice(0, 20));
    } catch (error) {
  ...
  const fetchAllCompanies = async (page = 1, filters = companyFilters) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/data-analysis/national-pension?${queryParams}`
      );
  ...
  useEffect(() => {
    const loadInitialCompanies = async () => {
      try {
        const response = await fetch(
          '/api/data-analysis/national-pension?type=all-companies&page=1&limit=100&sortBy=employees&sortOrder=desc'
        );
        const result = await response.json();
      ...
      <div className='min-h-screen bg-gray-900 p-8'>
        <div className='mx-auto max-w-7xl'>
          <div className='animate-pulse'>
            <div className='mb-8 h-8 w-1/3 rounded-lg bg-gray-700'></div>
            <div className='mb-8 grid grid-cols-1 gap-6 md:grid-cols-4'>
              {[1, 2, 3, 4].map(i => (
                <div key={i} className='h-32 rounded-lg bg-gray-800 p-6'></div>
              ))}
            </div>
            <div className='grid grid-cols-1 gap-8 lg:grid-cols-2'>
              {[1, 2, 3, 4].map(i => (
                <div key={i} className='h-96 rounded-lg bg-gray-800 p-6'></div>
              ))}
            </div>
          </div>
        </div>
  ...
  return (
    <div className='min-h-screen bg-gray-900 p-8'>
      <div className='mx-auto max-w-7xl'>
        {/* 헤더 */}
        <div className='mb-8'>
          <h1 className='mb-4 bg-gradient-to-r from-blue-400 to-purple-600 bg-clip-text text-4xl font-bold text-transparent text-white'>
            🏢 국민연금 사업장 분석 대시보드
          </h1>
        ...
        {overviewData && (
          <div className='mb-8 grid grid-cols-1 gap-6 md:grid-cols-4'>
            <div className='rounded-xl border border-blue-700 bg-gradient-to-br from-blue-900 to-blue-800 p-6 shadow-xl'>
            ...
            <div className='rounded-xl border border-green-700 bg-gradient-to-br from-green-900 to-green-800 p-6 shadow-xl'>
              <div className='flex items-center'>
            ...
            <div className='rounded-xl border border-purple-700 bg-gradient-to-br from-purple-900 to-purple-800 p-6 shadow-xl'>
              <div className='flex items-center'>
            ...
            <div className='rounded-xl border border-orange-700 bg-gradient-to-br from-orange-900 to-orange-800 p-6 shadow-xl'>
              <div className='flex items-center'>
                ...
                <div className='ml-4'>
                  ...
                  <p className='text-2xl font-bold text-white'>
                    {(
                      (overviewData.activeWorkplaces /
                        overviewData.totalWorkplaces) *
                      100
                    ).toFixed(1)}
                    %
                  </p>
                    ...
                    {industryData.map((item, index) => (
                      ...
                      >
                        ...
                        <td className='px-6 py-4 text-right'>
                          <span className='rounded bg-blue-900 px-2.5 py-0.5 text-xs font-medium text-blue-300'>
                            {(
                              (item.count / overviewData!.totalWorkplaces) *
                              100
                            ).toFixed(2)}
                            %
                          </span>
                      ...
                      <div className='space-y-2'>
                        ...
                        <div>
                          ...
                          <p className='text-xl font-bold text-green-400'>
                            {item.totalEmployees.toLocaleString()}명
                          </p>
                        </div>
                        <div>
                          ...
                          <p className='text-lg font-bold text-yellow-400'>
                            {item.totalEmployees > 0
                              ? Math.round(item.totalEmployees / item.count)
                              : 0}
                            명
                          </p>
                        </div>
            ...
            <div className='rounded-xl border border-gray-700 bg-gray-800 p-6 shadow-xl'>
                  ...
                  <tbody>
                    {topCompanies.map((company, index) => (
                      ...
                      >
                        ...
                        <td
                          className='max-w-xs truncate px-4 py-4 font-medium text-white'
                          title={company.name}
                        >
                          {company.name}
                        </td>
                        <td
                          className='max-w-xs truncate px-4 py-4 text-gray-400'
                        ...
                        <td className='px-4 py-4'>
                          <span className='rounded bg-purple-900 px-2.5 py-0.5 text-xs font-medium text-purple-300'>
                            {company.region}
                          </span>
                        </td>
                      </tr>
              ...
              <div className='mt-4 text-center'>
                <button
                  ...
                  className='rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-3 font-medium text-white shadow-lg transition-all duration-200 hover:from-blue-700 hover:to-purple-700'
                >
                  📋 전체 기업 목록 보기 (54만+ 기업)
                </button>
              </div>
        ...
        {activeTab === 'all-companies' && (
          <div className='space-y-8'>
            ...
            <div className='rounded-xl border border-gray-700 bg-gray-800 p-6 shadow-xl'>
                ...
                <div>
                  ...
                  <div className='flex gap-2'>
                    <select
                      ...
                      className='flex-1 rounded-md border border-gray-600 bg-gray-700 px-3 py-2 text-white focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500'
                    >
              ...
              <div className='flex items-center justify-between text-sm text-gray-400'>
                <span>
                  ...
                  {(companyPagination.page - 1) * companyPagination.limit + 1}-
                  {Math.min(
            ...
            {/* 전체 기업 데이터 그리드 */}
            <div className='rounded-xl border border-gray-700 bg-gray-800 shadow-xl'>
              ...
              {companiesLoading ? (
                ...
                <div className='flex h-64 flex-col items-center justify-center text-gray-400'>
                  ...
                  <p className='mt-1 text-sm'>필터 조건을 조정해보세요</p>
                </div>
              ) : (
                <div className='overflow-x-auto'>
                  <table className='w-full text-sm text-gray-300'>
                        ...
                        >
                          ...
                          <td className='px-4 py-4 text-right font-mono text-yellow-400'>
                            {company.monthlyAmount > 0
                              ? (company.monthlyAmount / 100000000).toFixed(1) +
                                '억원'
                              : '-'}
                          </td>
                          <td className='px-4 py-4'>
                ...
                companyPagination.pages > 1 && (
                  <div className='flex items-center justify-between border-t border-gray-700 px-6 py-4'>
                    <div className='flex items-center gap-2'>
                      ...
                      <button
                        onClick={() =>
                          handlePageChange(companyPagination.page - 1)
                        }
                        disabled={companyPagination.page <= 1}
                        className='rounded border border-gray-600 px-3 py-1 text-sm text-gray-300 transition-colors hover:bg-gray-600 disabled:cursor-not-allowed disabled:opacity-50'
                      >
                    ...
                    <div className='flex items-center gap-1'>
                      {Array.from(
                        { length: Math.min(5, companyPagination.pages) },
                        (_, i) => {
                          const pageNum =
                            Math.max(1, companyPagination.page - 2) + i;
                    ...
                    <div className='flex items-center gap-2'>
                      <button
                        ...
                        disabled={
                          companyPagination.page >= companyPagination.pages
                        }
                        className='rounded border border-gray-600 px-3 py-1 text-sm text-gray-300 transition-colors hover:bg-gray-600 disabled:cursor-not-allowed disabled:opacity-50'
                      >
                        다음
                      ...
                      <button
                        onClick={() =>
                          handlePageChange(companyPagination.pages)
                        }
                        disabled={
                          companyPagination.page >= companyPagination.pages
                        }
        ...
        {activeTab === 'keyword-graph' && (
          <div className='space-y-8'>
            <KeywordGraphSection className='rounded-xl border border-gray-700 bg-gray-800 p-6 shadow-xl' />
          </div>
        )}
      </div>
    </div>
