...
export function SeoulRestaurantsAnalysis() {
  ...
  const [businessStatusData, setBusinessStatusData] = useState<BusinessStatusData[]>([])
  ...
  const loadData = async () => {
    try {
      ...
      // 병렬로 모든 데이터 로드
      const [overviewRes, businessTypeRes, businessStatusRes, timelineRes, detailRes] = await Promise.all([
        ...
        fetch(`/api/data-analysis/seoul-restaurants?type=details&page=1&limit=50&search=${searchTerm}&business_type=${selectedBusinessType}&business_status=${selectedBusinessStatus}`)
      ])

      const [overview, businessType, businessStatus, timeline, detail] = await Promise.all([
        overviewRes.json(),
  ...
  return (
    <div className="space-y-8 bg-gray-900 min-h-screen p-6 rounded-lg">
          ...
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-white">{formatNumber(overviewData.activeBusinesses)}</p>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* 영업상태별 분포 파이차트 */}
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700">
          <h3 className="text-lg font-semibold mb-4 text-white">영업상태별 분포</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
      ...
      <div className="bg-gray-800 rounded-lg shadow-lg border border-gray-700">
        <div className="p-6 border-b border-gray-600">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">영업상태 필터</label>
              <select
          ...
          <table className="w-full">
            ...
            <tbody className="bg-gray-800 divide-y divide-gray-600">
              {detailData.map((item, index) => (
                <tr key={`${item.management_number}-${index}`} className="hover:bg-gray-700 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`
                      ...
                      ${item.business_status_name === '영업' ? 'bg-green-800 text-green-200' :
                        item.business_status_name === '폐업' ? 'bg-red-800 text-red-200' :
                        item.business_status_name === '휴업' ? 'bg-yellow-800 text-yellow-200' :
