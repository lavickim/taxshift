...
export function KeywordRuleTest() {
  ...
  const [testResults, setTestResults] = useState<
    Record<string, KeywordClassificationResult>
  >({});
  ...
  const [singleTestResult, setSingleTestResult] =
    useState<KeywordClassificationResult | null>(null);

  // 테스트 트랜잭션 데이터
  const [testTransactions, setTestTransactions] = useState<
    TestTransactionString[]
  >([]);
  ...
  const getProcessingPathIcon = (path: string) => {
    const icons: { [key: string]: React.ReactNode } = {
      cache: <Clock className='h-3 w-3' />,
      keyword: <Hash className='h-3 w-3' />,
      ml: <TrendingUp className='h-3 w-3' />,
      llm: <Server className='h-3 w-3' />,
    };
    return icons[path] || <Server className='h-3 w-3' />;
  };
  ...
  const loadSystemStats = async () => {
    try {
      const response = await fetch('/api/v2/tag-mapping-mgmt/stats');
      if (response.ok) {
        const data = await response.json();
        setSystemStats(data);
      }
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {systemStats.averageProcessingTime}ms
            </div>
          ...
          <div className='flex space-x-2'>
            <div className='flex-1'>
              ...
              <Input
                ...
                onKeyPress={e =>
                  e.key === 'Enter' && testSingleClassification(testInput)
                }
              />
            </div>
            <Button
            ...
            <div className='space-y-3 rounded-lg bg-gray-50 p-4'>
              <div className='flex items-center gap-2'>
                ...
                {singleTestResult.processingPath && (
                  <Badge
                    className={getProcessingPathColor(
                      singleTestResult.processingPath
                    )}
                  >
              ...
              {singleTestResult.matched && (
                <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
                  <div>
                    ...
                    <p className='text-sm'>
                      {singleTestResult.keywordGroup || '없음'}
                    </p>
                  </div>
                  <div>
                    <Label className='text-sm font-medium'>추출된 키워드</Label>
                    <div className='mt-1 flex flex-wrap gap-1'>
                      {singleTestResult.extractedKeywords?.map(
                        (keyword, idx) => (
                          <Badge
                            key={idx}
                            variant='secondary'
                  ...
                  <div>
                    ...
                    <p className='text-sm'>
                      {singleTestResult.accountCode} -{' '}
                      {singleTestResult.accountName}
                    </p>
                  </div>
                  <div>
                    <Label className='text-sm font-medium'>신뢰도</Label>
                    <p className='text-sm'>
                      {Math.round((singleTestResult.confidence || 0) * 100)}%
                    </p>
                  </div>
                  <div>
                    <Label className='text-sm font-medium'>처리 시간</Label>
                    <p className='text-sm'>
                      {singleTestResult.processingTime || 0}ms
                    </p>
                  </div>
                </div>
              )}
                ...
                {transactionData.transactions.map(transaction => {
                  ...
                  return (
                    <TableRow key={transaction.id}>
                      ...
                      <TableCell>{result?.keywordGroup || '-'}</TableCell>
                      <TableCell>
                        {result?.tag ? (
