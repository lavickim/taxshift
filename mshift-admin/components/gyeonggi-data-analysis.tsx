...
export function GyeonggiDataAnalysis() {
  ...
  const loadData = async () => {
    try {
      ...
      // 병렬로 모든 데이터 로드
      const [overviewRes, industryRes, regionRes, timelineRes, detailRes] =
        await Promise.all([
          fetch('/api/data-analysis/gyeonggi-delivery?type=overview'),
          fetch('/api/data-analysis/gyeonggi-delivery?type=industry'),
          fetch('/api/data-analysis/gyeonggi-delivery?type=region'),
          fetch('/api/data-analysis/gyeonggi-delivery?type=timeline'),
          fetch(
            `/api/data-analysis/gyeonggi-delivery?type=details&page=1&limit=50&search=${searchTerm}&industry=${selectedIndustry}&region=${selectedRegion}`
          ),
        ]);
  ...
  const loadDetailData = async (page = 1) => {
    try {
      const response = await fetch(
        `/api/data-analysis/gyeonggi-delivery?type=details&page=${page}&limit=50&search=${searchTerm}&industry=${selectedIndustry}&region=${selectedRegion}`
      );
        ...
        <div className='grid grid-cols-1 gap-6 md:grid-cols-4'>
          <div className='rounded-lg border border-gray-700 bg-gray-800 p-6 shadow-lg'>
            <div className='flex items-center'>
              ...
              <div className='ml-4'>
                <p className='text-sm font-medium text-gray-300'>총 레코드</p>
          ...
          <table className='w-full'>
            ...
            <tbody className='divide-y divide-gray-600 bg-gray-800'>
              {detailData.map((item, index) => (
                <tr
                  key={`${item.business_reg_no}-${index}`}
