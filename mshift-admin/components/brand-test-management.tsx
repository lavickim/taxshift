...
export default function BrandTestManagement() {
    ...
    try {
      const response = await fetch('/api/brands/batch-test', {
        ...
        body: JSON.stringify({
          ...
          batchSize,
          filters:
            filters.testStatus === 'untested' ? { testStatus: 'untested' } : {},
        }),
      });
    ...
    const interval = setInterval(async () => {
      try {
        const response = await fetch(
          `/api/brands/batch-status?batchId=${batchId}`
        );
        if (response.ok) {
          ...
          if (status.status === 'completed' || status.status === 'failed') {
            ...
            if (status.status === 'completed') {
              toast.success(
                `배치 테스트 완료: ${status.successCount}/${status.totalCount} 성공`
              );
            } else {
  ...
  const getTestResultIcon = (testPassed?: boolean) => {
    if (testPassed === undefined) return <Clock className='h-3 w-3' />;
    return testPassed ? (
  ...
  const showTestResultDetails = (brand: FranchiseBrand) => {
    ...
    if (
      result.rawResult?.extractedKeywords &&
      result.rawResult.extractedKeywords.length > 0
    ) {
  ...
  const successRate =
    statistics.testedBrands > 0
      ? (statistics.passedBrands / statistics.testedBrands) * 100
      : 0;
    ...
    <div className='space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='flex items-center gap-2'>
          <Button
            onClick={generateTransactionStrings}
            disabled={loading}
            variant='outline'
          >
          ...
          <Button
            onClick={() => startBatchTest(100)}
            disabled={loading || currentBatch?.status === 'running'}
          >
      ...
      <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4'>
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {statistics.totalBrands.toLocaleString()}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {statistics.testedBrands.toLocaleString()}
            </div>
            <p className='text-xs text-muted-foreground'>
              {statistics.totalBrands > 0 &&
                `${Math.round((statistics.testedBrands / statistics.totalBrands) * 100)}% 완료`}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {statistics.failedBrands.toLocaleString()}
            </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className='flex items-center gap-2'>
              <RefreshCw
                className={`h-4 w-4 ${currentBatch.status === 'running' ? 'animate-spin' : ''}`}
              />
              배치 테스트 진행 상황
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className='space-y-4'>
              ...
              <Progress
                value={
                  (currentBatch.processedCount / currentBatch.totalCount) * 100
                }
              />
              <div className='grid grid-cols-3 gap-4 text-sm'>
                ...
                <div className='text-center'>
                  <div className='font-medium'>
                    {currentBatch.totalCount - currentBatch.processedCount}
                  </div>
          ...
          <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4'>
            <div>
              ...
              <Select
                ...
                onValueChange={value =>
                  setFilters(prev => ({ ...prev, testStatus: value }))
                }
              >
                <SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={value =>
                  setFilters(prev => ({ ...prev, primaryTag: value }))
                }
              >
                <SelectTrigger>
                ...
                <TableRow key={brand.id}>
                  ...
                  <TableCell>
                    <Badge
                      ...
                      onClick={() =>
                        brand.testResult && showTestResultDetails(brand)
                      }
                    >
                      {getTestResultIcon(brand.testPassed)}
                      <span className='ml-1'>
                        {brand.testPassed === undefined
                          ? '미테스트'
                          : brand.testPassed
                            ? '성공'
                            : '실패'}
