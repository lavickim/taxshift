...
export function SegmentedKeywordAnalysis({}: SegmentedKeywordAnalysisProps) {
  ...
  const segmentTypes = {
    category: {
      ...
      color: 'bg-blue-500',
      segments: ['제조업', '음식점', '의료', '교육', '소매', '건설/부동산', '운송', '미용', '서비스업', '기타']
    },
    region: {
      name: '지역별',
      icon: <MapPin className="w-4 h-4" />,
      color: 'bg-green-500',
      segments: ['11', '26', '27', '28', '29', '30', '31', '41', '42', '43', '44', '45', '46', '47', '48', '50']
    },
    size: {
      name: '규모별',
  ...
  const loadStatistics = async () => {
    try {
      const response = await fetch(`${API_BASE}/v2/segmented-keywords/segments/statistics`)
      if (response.ok) {
  ...
  // 세그먼트별 키워드 로드
  const loadSegmentKeywords = async (segmentType: string, segmentName?: string) => {
    setLoading(true)
    setError(null)
    
    try {
      const params = new URLSearchParams({ limit: '30' })
      if (segmentName) params.append('segmentName', segmentName)
      
      const response = await fetch(`${API_BASE}/v2/segmented-keywords/segments/${segmentType}/keywords?${params}`)
      ...
      setError(err instanceof Error ? err.message : '키워드 로드 중 오류가 발생했습니다.')
      setKeywords([])
    } finally {
      setLoading(false)
    }
  ...
  const loadRelatedKeywords = async (keyword: string) => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`${API_BASE}/v2/segmented-keywords/keywords/${encodeURIComponent(keyword)}/related?limit=10`)
  ...
  // 그래프 데이터 로드
  const loadGraphData = async (segmentType: string, segmentName?: string, keyword?: string) => {
    setLoading(true)
    try {
      ...
      const response = await fetch(`${API_BASE}/v2/segmented-keywords/graph?${params}`)
  ...
  const renderD3Graph = (data: {nodes: GraphNode[], links: GraphLink[]}) => {
    ...
    data.nodes.forEach((node, index) => {
      ...
      circle.setAttribute('r', Math.min(20, 5 + node.frequency / 10).toString())
      ...
      text.textContent = node.id.length > 8 ? node.id.substring(0, 8) + '...' : node.id
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h2 className="text-2xl font-bold tracking-tight">세그먼트 키워드 분석</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {statistics.map((stat) => {
          const segmentConfig = segmentTypes[stat.segment_type as keyof typeof segmentTypes]
          return (
            <Card key={stat.segment_type} className="cursor-pointer hover:shadow-md transition-shadow"
                  onClick={() => setActiveSegmentType(stat.segment_type)}>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">
                  {segmentConfig?.name || stat.segment_type}
                </CardTitle>
                <div className={`p-1 rounded ${segmentConfig?.color || 'bg-gray-500'} text-white`}>
                  {segmentConfig?.icon || <BarChart3 className="w-4 h-4" />}
                </div>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{stat.keyword_count.toLocaleString()}</div>
                <p className="text-xs text-muted-foreground">
                  {stat.segment_count}개 세그먼트 • {(stat.avg_confidence * 100).toFixed(1)}% 신뢰도
                </p>
              </CardContent>
            </Card>
          )
      ...
      {/* 메인 분석 영역 */}
      <Tabs value={activeSegmentType} onValueChange={setActiveSegmentType} className="space-y-4">
        ...
        {Object.entries(segmentTypes).map(([segmentType, config]) => (
          <TabsContent key={segmentType} value={segmentType} className="space-y-4">
                  ...
                  <CardTitle className="flex items-center gap-2">
                    ...
                    {selectedSegment && (
                      <Badge variant="secondary">
                        {segmentType === 'region' ? 
                          regionNames[selectedSegment as keyof typeof regionNames] || selectedSegment :
                          selectedSegment}
                      </Badge>
                    )}
                  </CardTitle>
                    ...
                    <div className="space-y-2 max-h-96 overflow-y-auto">
                      {keywords.map((keyword, index) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-sm text-gray-500">{keyword.count}회</span>
                            <Badge variant="secondary">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {relatedKeywords.map((related, index) => (
                      <Badge
                        ...
                        onClick={() => setSearchKeyword(related.related_keyword)}
                      >
