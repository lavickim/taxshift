...
import { 
  Play, AlertCircle, Cpu, CheckCircle, XCircle, HelpCircle, 
  Sparkles, TrendingUp, AlertTriangle, MessageSquare, Lightbulb, 
  Layers, Edit, Plus, Brain, Target, Zap, BarChart3, Settings2
} from 'lucide-react';
...
export function LLMManagement() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="hybrid" className="space-y-4" onValueChange={setCurrentTab}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-lg">하이브리드 엔진 테스트</CardTitle>
                    <p className="text-sm text-muted-foreground">
                      거래 내역을 입력하여 하이브리드 엔진의 분류 결과를 테스트해보세요
                    </p>
                  </CardHeader>
                  <CardContent>
                    ...
                    {llmResult.success && llmResult.data ? (
                      <Card className="border-blue-500">
                        ...
                        <CardContent className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              <p className="font-medium text-sm">분류 결과</p>
                              <p className="text-lg">{llmResult.data.classification}</p>
                            </div>
                            <div>
                              <p className="font-medium text-sm">신뢰도</p>
                              <div className="flex items-center gap-2">
                                <div className="w-24 h-2 bg-gray-200 rounded-full overflow-hidden">
                                  <div 
                                    className={`h-full ${
                                      llmResult.data.confidence >= 80 ? 'bg-green-500' :
                                      llmResult.data.confidence >= 60 ? 'bg-yellow-500' :
                                      'bg-red-500'
                                    }`}
                                    style={{ width: `${llmResult.data.confidence}%` }}
                                  />
                                </div>
                                <span className="font-medium">{llmResult.data.confidence}%</span>
                              </div>
                            </div>
                          </div>
                          
                          <div>
                            <p className="font-medium text-sm mb-2">추론 과정</p>
                            <div className="bg-muted p-3 rounded text-sm">
                              {llmResult.data.reasoning}
                            </div>
                          </div>
                          
                          {llmResult.data.alternative_options && llmResult.data.alternative_options.length > 0 && (
                            <div>
                              <p className="font-medium text-sm mb-2">대안 분류</p>
                              <div className="space-y-2">
                                {llmResult.data.alternative_options.map((option, idx) => (
                                  <Card key={idx} className="border-l-4 border-l-blue-300">
                                    <CardContent className="p-3">
                                      <div className="flex justify-between items-start">
                                        <div>
                                          <p className="font-medium">{option.tag} / {option.account}</p>
                                          <p className="text-sm text-muted-foreground">{option.reasoning}</p>
                                        </div>
                                        <Badge variant="secondary">{option.confidence}%</Badge>
                                      </div>
                                    </CardContent>
                                  </Card>
                                ))}
            ...
            <CardContent>
              {feedback.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">룰 엔진 테스트에서 피드백을 제공하면 여기에 표시됩니다.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {feedback.map((item) => (
                        ...
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <Badge variant={
                                item.feedback_type === 'positive' ? 'default' : 
                                item.feedback_type === 'negative' ? 'destructive' : 'secondary'
                              }>
                                {item.feedback_type === 'positive' ? '긍정' : 
                                 item.feedback_type === 'negative' ? '부정' : 'LLM'}
                              </Badge>
                              <span className="text-sm text-muted-foreground">
                                {new Date(item.created_at).toLocaleDateString()}
                              </span>
                            </div>
                            <p className="font-medium">{item.transaction_text}</p>
                            <div className="text-sm text-muted-foreground mt-2">
                              선택된 분류: {item.selected_tag} / {item.selected_account}
                            </div>
                          </div>
                        </div>
                      </CardContent>
              ...
              {analytics ? (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <Card className="border-l-4 border-l-blue-500">
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-medium text-muted-foreground">총 LLM 호출</p>
                            <p className="text-2xl font-bold text-blue-600">{analytics.total_llm_calls}</p>
                          </div>
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-2xl font-bold text-green-600">{analytics.avg_confidence}%</p>
                          </div>
                    ...
                    <Card className="border-l-4 border-l-orange-500">
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-medium text-muted-foreground">피드백 총계</p>
                            <p className="text-2xl font-bold text-orange-600">
                              {analytics.feedback_stats.positive + analytics.feedback_stats.negative + analytics.feedback_stats.llm}
                            </p>
                          </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <CardContent>
                        <ResponsiveContainer width="100%" height={300}>
                          <PieChart>
                            <Pie
                              data={[
                                { name: '긍정', value: analytics.feedback_stats.positive },
                                { name: '부정', value: analytics.feedback_stats.negative },
                                { name: 'LLM', value: analytics.feedback_stats.llm }
                              ]}
                              cx="50%"
                              cy="50%"
                              labelLine={false}
                    ...
                    <Card>
                      <CardHeader>
                        <CardTitle className="text-lg">주요 분류 항목</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <ResponsiveContainer width="100%" height={300}>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">AI 사용량 데이터를 불러오고 있습니다.</p>
                </div>
              )}
            </CardContent>
          </Card>
