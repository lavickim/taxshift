    ...
    return NextResponse.json(
      {
        success: false,
        error:
          'Invalid request format. Provide either "text" or "transactions" array.',
      },
      { status: 400 }
    );
  } catch (error) {
...
function generateBatchSummary(results: any[]) {
  ...
  const preprocessingSuccess = results.filter(
    r => r.preprocessingSuccess
  ).length;
  ...
  const avgProcessingTime =
    results.reduce((sum, r) => sum + r.totalProcessingTime, 0) / total;
  const avgConfidence =
    results.reduce((sum, r) => sum + r.finalConfidence, 0) / total;
