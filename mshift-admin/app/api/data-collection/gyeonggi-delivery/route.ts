...
export async function POST(request: NextRequest) {
  try {
    ...
    const insertData = {
      ...
      refined_latitude: data.refined_latitude
        ? parseFloat(data.refined_latitude)
        : null,
      refined_longitude: data.refined_longitude
        ? parseFloat(data.refined_longitude)
        : null,
  ...
  try {
    ...
    if (action === 'status') {
      try {
        ...
        // 수집 재개를 위한 추가 정보
        const lastBatchInfo = await prisma.$queryRaw<
          Array<{
            collection_batch_id: string;
            max_created_at: Date;
            record_count: bigint;
          ...
          status: {
            ...
            resumeInfo: {
              ...
              estimatedPages: Math.ceil(
                Number(totalCount[0]?.count || 0) / 1000
              ),
              suggestedStartPage: Math.max(
                1,
                Math.ceil(Number(totalCount[0]?.count || 0) / 1000) - 1
              ), // 마지막 페이지 이전부터 시작 (오버랩)
            },
          },
        });
