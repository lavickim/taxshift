...
// 사업자번호 추출 및 정리 함수
function extractBusinessNumber(managementNumber: string, localGovCode: string): string {
...
async function loadCSVToMemory(): Promise<SeoulRestaurantData[]> {
  ...
  const mainFilePath = path.join(process.cwd(), 'data', '서울시 일반음식점 인허가 정보.csv')
  
  // 정식 파일이 있으면 사용
  if (fs.existsSync(mainFilePath)) {
    addLog(`📂 정식 CSV 파일 로드 시작: 서울시 일반음식점 인허가 정보.csv (198MB, CP949 인코딩)`)
    return readCSVFileFromPath(mainFilePath)
  }
  
  // 정식 파일이 없으면 이전 파일명으로 시도
  const legacyFilePath = path.join(process.cwd(), 'data', '서울시_일반음식점.csv')
  ...
  // 둘 다 없으면 샘플 파일 사용
  const samplePath = path.join(process.cwd(), 'data', '서울시_일반음식점_샘플.csv')
...
// 지정된 경로에서 CSV 파일 읽기
async function readCSVFileFromPath(filePath: string): Promise<SeoulRestaurantData[]> {
    ...
    fs.createReadStream(filePath)
      ...
      .on('data', (data) => {
        ...
        if (rowCount <= 5) {
          ...
          // 사업자번호 추출 테스트
          const testBusinessNumber = extractBusinessNumber(data['관리번호'] || '', data['개방자치단체코드'] || '');
          addLog(`  - 추출된 사업자번호: "${testBusinessNumber}" (길이: ${testBusinessNumber.length})`)
        }
        ...
        if (businessStatus === '폐업') {
          ...
          if (processingStatus.skippedRecords % 1000 === 0) {
            addLog(`⏭️ 폐업 데이터 스킵: ${processingStatus.skippedRecords.toLocaleString()}건 제외됨`);
          }
      ...
      .on('end', () => {
        addLog(`✅ CSV 파일 로드 완료: 총 ${results.length.toLocaleString()}건 (폐업 ${processingStatus.skippedRecords.toLocaleString()}건 제외됨)`)
...
async function processBatch(batch: SeoulRestaurantData[], batchNumber: number, batchId: string, prisma: any) {
  let batchSaved = 0
  let batchErrors = 0
  let batchSkipped = 0
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    if (processingStatus.pauseRequested) {
      ...
      return { saved: batchSaved, errors: batchErrors, skipped: batchSkipped, paused: true }
    }
    ...
    try {
      ...
      if (!restaurant.사업장명 && !restaurant.관리번호) {
        ...
        currentBatchData[i].error = '필수 데이터 부족 (사업장명과 관리번호 모두 없음)'
        batchErrors++
        if (i < 3) { // 처음 3개만 로그
          addLog(`❌ 필수 데이터 부족: 사업장명="${restaurant.사업장명}", 관리번호="${restaurant.관리번호}"`)
        }
        continue
      }
      ...
      if (restaurant.관리번호) {
        cleanedBusinessNumber = extractBusinessNumber(restaurant.관리번호, restaurant.개방자치단체코드)
      }
      
      // 사업자번호가 없거나 유효하지 않은 경우 관리번호 원본 사용
      if (!cleanedBusinessNumber || cleanedBusinessNumber.length !== 10) {
        cleanedBusinessNumber = restaurant.관리번호 || `temp_${Date.now()}_${i}`
        if (i < 3) {
          addLog(`⚠️ 사업자번호 추출 실패, 원본 사용: "${restaurant.관리번호}" -> "${cleanedBusinessNumber}"`)
        }
      }
      ...
      // 첫 3건은 성공 로그 표시
      if (i < 3) {
        addLog(`✅ 저장 완료: ${restaurant.사업장명} (${cleanedBusinessNumber})`)
        addLog(`  📍 좌표: X=${restaurant.좌표정보X}, Y=${restaurant.좌표정보Y}`)
      ...
      batchErrors++
      if (i < 5) { // 처음 5개 오류만 자세히 로그
        addLog(`💥 저장 오류: ${restaurant.사업장명}`)
        addLog(`  ❌ 에러: ${error.message}`)
        addLog(`  📝 관리번호: "${restaurant.관리번호}" -> "${cleanedBusinessNumber}"`)
        addLog(`  📍 좌표: X="${restaurant.좌표정보X}", Y="${restaurant.좌표정보Y}"`)
  ...
  addLog(`✅ 배치 ${batchNumber} 완료: 저장 ${batchSaved}건, 제외 ${batchSkipped}건, 오류 ${batchErrors}건`)
  
  return { saved: batchSaved, errors: batchErrors, skipped: batchSkipped, paused: false }
}
    ...
    if (action === 'status') {
      ...
      return NextResponse.json({
        ...
        processing: {
          ...
          progress: processingStatus.totalRecords > 0 ? 
            Math.round((processingStatus.processedRecords / processingStatus.totalRecords) * 100) : 0
        }
      })
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'process-to-db') {
      ...
      addLog(`🚀 메모리 데이터 DB 저장 시작 (${memoryData.length.toLocaleString()}건)`)
...
async function loadCSVInBackground() {
  try {
    ...
    const duration = Math.round((endTime.getTime() - (processingStatus.startTime?.getTime() || 0)) / 1000)
    
    addLog(`🎉 CSV 파일 로드 완료!`)
    addLog(`📊 총 ${memoryData.length.toLocaleString()}건이 메모리에 로드됨`)
    if (processingStatus.skippedRecords > 0) {
      addLog(`⏭️ 폐업 데이터 ${processingStatus.skippedRecords.toLocaleString()}건 제외됨`)
    }
...
async function processMemoryDataToDb() {
  try {
    ...
    addLog(`📦 총 ${batches.length}개의 배치로 나누어 처리 (배치당 ${BATCH_SIZE}건)`)
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      const result = await processBatch(batches[i], i + 1, processingStatus.batchId, prisma)
    ...
    addLog(`🎉 DB 저장 처리 완료!`)
    addLog(`📊 최종 결과: 처리 ${processingStatus.processedRecords.toLocaleString()}건`)
