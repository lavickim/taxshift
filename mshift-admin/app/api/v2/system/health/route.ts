...
async function checkCriticalEndpoints() {
  const endpoints = [
    { name: 'keyword-groups', url: '/api/v2/tag-mapping-mgmt/keyword-groups' },
    { name: 'tag-stats', url: '/api/v2/tag-mapping-mgmt/stats' },
    {
      name: 'recommend-tags',
      url: '/api/v2/tag-mapping-mgmt/recommend-tags',
      method: 'POST',
    },
  ];
    ...
    for (const testCase of testCases) {
      try {
        const response = await fetch(
          `${process.env.NEXTAUTH_URL}/api/v2/tag-mapping-mgmt/recommend-tags`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        console.warn(
          `키워드 엔진 테스트 케이스 실패: ${testCase.input}`,
          error.message
        );
      }
    }
