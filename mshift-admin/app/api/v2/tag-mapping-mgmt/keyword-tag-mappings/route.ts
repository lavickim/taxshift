...
export async function GET(request: NextRequest) {
  try {
    ...
    let endpoint = '/v2/tag-mapping-mgmt/keyword-tag-mappings';
    if (keywordGroupId) {
      endpoint = `/v2/tag-mapping-mgmt/keyword-groups/${keywordGroupId}/tag-mappings`;
    }

    console.log(
      'GET /api/v2/tag-mapping-mgmt/keyword-tag-mappings - 키워드-태그 매핑 목록 조회'
    );
    ...
    console.error('키워드-태그 매핑 조회 실패:', error);
    return NextResponse.json([], { status: 200 }); // Return empty array instead of error
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      'POST /api/v2/tag-mapping-mgmt/keyword-tag-mappings - 키워드-태그 매핑 생성:',
      body
    );

    // 요청 데이터 검증
    const { keywordGroupId, tagId, confidenceScore, priority, contextRules } =
      body;
    ...
    const response = await fetch(
      `${JAVA_API_BASE_URL}/v2/tag-mapping-mgmt/keyword-tag-mappings`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
