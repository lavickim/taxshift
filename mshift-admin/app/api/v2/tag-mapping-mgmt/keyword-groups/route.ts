...
export async function GET(request: NextRequest) {
  try {
    console.log(
      'GET /api/v2/tag-mapping-mgmt/keyword-groups - 키워드 그룹 목록 조회'
    );

    const response = await fetch(
      `${JAVA_API_BASE_URL}/v2/tag-mapping-mgmt/keyword-groups`,
      {
        method: 'GET',
        headers: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      'POST /api/v2/tag-mapping-mgmt/keyword-groups - 키워드 그룹 생성:',
      body
    );

    // 요청 데이터 검증
    const { groupName, primaryKeyword, synonyms, category, confidenceBase } =
      body;
