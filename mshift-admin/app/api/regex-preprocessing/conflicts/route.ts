    ...
    return NextResponse.json({
      ...
      data: {
        ...
        summary: {
          ...
          criticalConflicts: conflicts.filter(c => c.impactLevel === 'critical').length,
          warningConflicts: conflicts.filter(c => c.impactLevel === 'warning').length,
          infoConflicts: conflicts.filter(c => c.impactLevel === 'info').length,
          averageOverlapPercentage: conflicts.length > 0 
            ? conflicts.reduce((sum, c) => sum + c.overlapPercentage, 0) / conflicts.length 
            : 0
        }
      }
    });
