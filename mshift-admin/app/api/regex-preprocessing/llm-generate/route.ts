...
export async function POST(request: NextRequest) {
  try {
    ...
    const { category, description, sampleInputs, expectedOutputs, context } = body;
    
    // 입력 검증
    if (!category || !description || !sampleInputs || sampleInputs.length === 0) {
    ...
    // 생성된 규칙 유효성 검증
    const validatedRules = await validateGeneratedRules(generatedRules, sampleInputs);
    
    // 충돌 분석
    const conflictAnalysis = await analyzePatternConflicts(validatedRules, existingRules);
    
    // 패턴 분석
    const analysisResult = analyzePatternComplexity(validatedRules);
    
    // 개선 제안 생성
    const suggestions = generatePatternSuggestions(validatedRules, conflictAnalysis);
...
function buildLLMPrompt(params: {
  ...
  const { category, description, sampleInputs, expectedOutputs, context, existingRules } = params;
...
function parseLLMResponse(llmResponse: string, category: string): GeneratedRule[] {
  try {
...
async function validateGeneratedRules(rules: GeneratedRule[], sampleInputs: string[]): Promise<GeneratedRule[]> {
  return rules.filter(rule => {
    try {
      ...
      // 최소 50% 이상 매칭되어야 유효
      rule.confidence = Math.min(rule.confidence, matchCount / sampleInputs.length);
...
async function analyzePatternConflicts(generatedRules: GeneratedRule[], existingRules: any[]): Promise<string[]> {
  const conflicts: string[] = [];
  
  for (const newRule of generatedRules) {
    for (const existingRule of existingRules) {
      // 단순한 충돌 감지 (실제로는 더 정교한 알고리즘 필요)
      if (newRule.inputPattern.includes(existingRule.inputPattern.slice(0, 10))) {
        conflicts.push(`${newRule.ruleName}이 기존 규칙 "${existingRule.ruleName}"과 충돌할 수 있습니다`);
      }
    }
  }
...
function analyzePatternComplexity(rules: GeneratedRule[]): PatternAnalysis {
  ...
  rules.forEach(rule => {
    ...
    const specialCharCount = (rule.inputPattern.match(/[.*+?^${}()|[\]\\]/g) || []).length;
    ...
    if (complexity > 5) {
      recommendations.push(`${rule.ruleName}: 패턴이 복잡합니다. 단순화를 고려하세요.`);
    }
    
    if (rule.confidence < 0.7) {
      recommendations.push(`${rule.ruleName}: 신뢰도가 낮습니다. 추가 테스트가 필요합니다.`);
    }
  });
  ...
  return {
    complexity: avgComplexity < 2 ? 'low' : avgComplexity < 4 ? 'medium' : 'high',
    potentialConflicts: [], // 다른 함수에서 채움
    estimatedAccuracy: avgAccuracy * 100,
    performanceImpact: avgComplexity < 2 ? 'minimal' : avgComplexity < 4 ? 'moderate' : 'significant',
    recommendations
  };
}

function generatePatternSuggestions(rules: GeneratedRule[], conflicts: string[]): PatternSuggestion[] {
  ...
  if (conflicts.length > 0) {
    suggestions.push({
      ...
      pattern: '',
      reasoning: '우선순위를 조정하거나 패턴을 더 구체적으로 만드는 것을 권장합니다'
    });
  }
