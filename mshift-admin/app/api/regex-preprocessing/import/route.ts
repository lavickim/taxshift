...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file.name.endsWith('.xlsx') && !file.name.endsWith('.xls')) {
      return NextResponse.json(
        { success: false, error: 'Invalid file type. Only Excel files are supported.' },
        { status: 400 }
      );
    }
    ...
    if (rawData.length < 2) {
      return NextResponse.json(
        { success: false, error: 'Excel file must contain header row and at least one data row' },
        { status: 400 }
      );
    }
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      try {
        ...
        const priority = parseInt(getCellValue(row, headers, '우선순위')) || 100;
...
function getCellValue(row: any[], headers: string[], fieldName: string): string {
