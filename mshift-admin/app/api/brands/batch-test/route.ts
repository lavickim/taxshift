  ...
  try {
    ...
    for (const brand of brands) {
      ...
      try {
        ...
        console.log(
          `✓ ${brand.brandName}: ${testResult.matched ? 'PASS' : 'FAIL'} (${processingTime}ms)`
        );
      } catch (error) {
    ...
    console.log(
      `Batch ${batchId} completed: ${batch.successCount}/${batch.totalCount} passed`
    );
  } catch (error) {
...
async function testBrandTransaction(brand: any) {
  ...
  try {
    ...
    const response = await fetch(
      'http://localhost:8080/v2/keyword-test/classify',
      {
        method: 'POST',
        headers: {
