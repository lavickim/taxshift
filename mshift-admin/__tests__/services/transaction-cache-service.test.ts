...
describe('TransactionCacheService 클래스 테스트', () => {
  ...
  describe('create() - 캐시 데이터 생성', () => {
    ...
    it('빈 문자열 입력 시 에러를 던져야 함', async () => {
      ...
      await expect(service.create(emptyData)).rejects.toThrow(
        'rawText는 비어있을 수 없습니다'
      );
    });
  });
  ...
  describe('findByHash() - 해시로 캐시 조회', () => {
    ...
    it('잘못된 해시 형식 시 에러를 던져야 함', async () => {
      await expect(service.findByHash('invalid')).rejects.toThrow(
        '해시는 64자리 16진수여야 합니다'
      );
    });
  ...
  describe('findAll() - 모든 캐시 조회', () => {
    ...
    it('모든 캐시 데이터를 조회할 수 있어야 함', async () => {
      ...
      expect(
        allCaches.some(cache => cache.rawText === '(주)부자 충주(상)주')
      ).toBe(true);
      expect(
        allCaches.some(cache => cache.rawText === '구글페이먼트코리아')
      ).toBe(true);
    });
    ...
    it('생성 순서대로 정렬되어 반환되어야 함', async () => {
      ...
      expect(allCaches[0].createdAt.getTime()).toBeLessThanOrEqual(
        allCaches[1].createdAt.getTime()
      );
    });
  });
  ...
  describe('delete() - 캐시 데이터 삭제', () => {
    ...
    it('존재하지 않는 해시 삭제 시도 시 에러를 던져야 함', async () => {
      ...
      await expect(service.delete(nonExistentHash)).rejects.toThrow(
        '해당 해시의 캐시를 찾을 수 없습니다'
      );
    });
  ...
  describe('upsert() - 캐시 데이터 생성 또는 업데이트', () => {
    it('존재하지 않는 데이터는 새로 생성해야 함', async () => {
      ...
      const upserted = await service.upsert(hash, testData, {
        uniqueKey: '업데이트키',
      });
  ...
  describe('성능 테스트', () => {
    ...
    it('조회 성능 최적화 테스트', async () => {
      ...
      const queries = Array.from({ length: 100 }, () =>
        service.findByHash('8'.repeat(64))
      );
