...
describe('General Ledger System (Phase 3)', () => {
  beforeAll(async () => {
    ...
    // 기본 계정과목 데이터 생성
    const baseAccounts = [
      { accountCode: '1110', accountName: '현금', accountType: '자산', isDebitNormal: true },
      { accountCode: '2110', accountName: '미지급금', accountType: '부채', isDebitNormal: false },
      { accountCode: '4110', accountName: '매출', accountType: '수익', isDebitNormal: false },
      { accountCode: '5310', accountName: '차량유지비', accountType: '비용', isDebitNormal: true },
      { accountCode: '5410', accountName: '소모품비', accountType: '비용', isDebitNormal: true }
    ];
  ...
  describe('GL Tables and Schema', () => {
    test('should have general_ledger table with required fields', async () => {
      ...
      const requiredColumns = [
        'id', 'company_id', 'account_code', 'fiscal_year', 'fiscal_month',
    ...
    test('should have gl_details table with required fields', async () => {
      ...
      const requiredColumns = [
        'id', 'general_ledger_id', 'journal_entry_id', 'posting_date',
    ...
    test('should have journal_entries table with required fields', async () => {
      ...
      const requiredColumns = [
        ...
        'status', 'confidence_score', 'posted_at', 'created_at', 'updated_at'
      ];
    ...
    test('should have journal_entry_details table with required fields', async () => {
      ...
      const requiredColumns = [
        'id', 'journal_entry_id', 'line_number', 'account_code', 'account_name',
  ...
  describe('General Ledger Posting', () => {
    ...
    test('should create GL detail records for audit trail', async () => {
      ...
      await prisma.glDetail.createMany({
        data: [
          {
            generalLedgerId: glLedgers.find(gl => gl.accountCode === '1110')!.id,
          ...
          {
            generalLedgerId: glLedgers.find(gl => gl.accountCode === '2110')!.id,
            journalEntryId: journalEntry.id,
            postingDate: new Date('2025-01-15'),
            debitAmount: 0,
            creditAmount: 100000,
      ...
      const cashGLDetail = glDetails.find(detail => Number(detail.debitAmount) === 100000);
      expect(cashGLDetail).toBeDefined();
      expect(Number(cashGLDetail?.creditAmount)).toBe(0);
      
      const payableGLDetail = glDetails.find(detail => Number(detail.creditAmount) === 100000);
    ...
    test('should calculate running balance correctly in GL details', async () => {
      ...
      const transactions = [
        { debit: 500000, credit: 0, runningBalance: 500000, description: '기초 현금' },
        { debit: 200000, credit: 0, runningBalance: 700000, description: '현금 매출' },
        { debit: 0, credit: 100000, runningBalance: 600000, description: '현금 지출' },
        { debit: 150000, credit: 0, runningBalance: 750000, description: '추가 입금' }
      ];
      ...
      expect(Number(glDetails[glDetails.length - 1].runningBalance)).toBe(750000);
    });
  });
  ...
  describe('Month-end Closing Process', () => {
    test('should close GL accounts at month-end', async () => {
      ...
      expect(closedRecords.every(record => record.closedAt !== null)).toBe(true);
    });
