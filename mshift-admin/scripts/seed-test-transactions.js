...
function loadFullTestCasesFromFile() {
  ...
  try {
    const testFilePath = path.join(__dirname, '../../project-design/test-transaction-strings-1000plus.md');
...
function extractTestCases(section, count, startIndex = 0) {
  ...
  for (let i = 0; i < lines.length && cases.length < count; i++) {
    ...
    if (line && !line.startsWith('#') && !line.startsWith('```') && !line.startsWith('*') && line.length > 3) {
...
function getSimplifiedTestCases() {
  ...
  return {
    '편의점': ['세븐일레븐 강남점', '7-ELEVEN 신촌점', 'CU 홍대점', 'GS25 종로점', '이마트24 서초점'],
    '주유소': ['GS칼텍스 강남주유소', 'SK에너지 서초주유소', '현대오일뱅크 여의도주유소', 'S-Oil 송파주유소'],
    '카페': ['스타벅스 강남역점', '투썸플레이스 신촌점', '이디야커피 압구정점', '커피빈 강남점'],
    '패스트푸드': ['맥도날드 강남점', '롯데리아 명동점', '버거킹 압구정점', 'KFC 종로점'],
    '치킨': ['BBQ 노원점', '굽네치킨 강북점', '네네치킨 구로점', '교촌치킨 서대문점'],
    '마트': ['이마트 강남점', '롯데마트 명동점', '홈플러스 잠실점', '코스트코 강남점'],
  ...
  try {
    ...
    for (const [category, texts] of Object.entries(TEST_CASES_BY_CATEGORY)) {
      ...
      for (const originalText of texts) {
        ...
        const extendedText = generateExtendedTransactionText(originalText, amount);
    ...
    for (let i = 0; i < testData.length; i += chunkSize) {
      ...
      console.log(`   진행: ${Math.min(i + chunkSize, testData.length)}/${testData.length}`);
    }
    ...
    console.log(`📋 카테고리 수: ${Object.keys(TEST_CASES_BY_CATEGORY).length}개`);
    ...
    samples.forEach((sample, index) => {
      ...
      console.log(`   카테고리: ${sample.category} | 금액: ${sample.formattedAmount}`);
      console.log(`   예상 태그: ${sample.expectedTag} | 계정: ${sample.expectedAccountCode}-${sample.expectedAccountName}\n`);
    });
