...
function runMVPTest() {
  ...
  console.log(`\n📈 키워드 패턴 진화 과정:`);
  console.log(
    `46% (기본) → 55% (향상) → 62% (최종) → ${successRate}% (MVP준비)`
  );
  ...
  sampleResults.forEach((sample, index) => {
    ...
    console.log(
      `   → ${sample.tag} | ${sample.accountCode}-${sample.accountName}`
    );
    console.log(
      `   → 키워드: "${sample.matchedKeyword}" | 신뢰도: ${Math.round(sample.confidence * 100)}%\n`
    );
  });
  ...
  console.log(
    `  • 작업 시간 절약: ${Math.round((automatedCount * 30) / 3600)}시간/일 (거래당 30초 가정)`
  );
  ...
  if (successRate < 75) {
    mvpResults.recommendations.push(
      '75% 목표 달성을 위해 추가 키워드 패턴이 필요합니다.'
    );
    mvpResults.recommendations.push(
      '특히 미매칭 케이스에서 자주 나타나는 브랜드들의 패턴 추가를 권장합니다.'
    );
  } else {
    mvpResults.recommendations.push('MVP 런칭 준비가 완료되었습니다!');
    mvpResults.recommendations.push(
      '지속적인 모니터링을 통해 새로운 브랜드 패턴을 추가하세요.'
    );
  }
  ...
  fs.writeFileSync(
    path.join(outputDir, 'mvp-ready-results.json'),
    JSON.stringify(mvpResults, null, 2)
  );
