...
async function createKeywordGroupsWithPrompts() {
  ...
  for (const group of industryGroups) {
    ...
    const prompt = createKeywordGroupPrompt(brands, group.industryMediumCategory);
    const keywordGroup = await simulateKeywordGroupAnalysis(prompt, brands, group.industryMediumCategory);
