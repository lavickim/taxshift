...
function inferTagsFromIndustry(industryLargeCategory, industryMediumCategory, brandName) {
  let tags = [];
  ...
  } else if (brandLower.includes('카페') || brandLower.includes('coffee') || brandLower.includes('커피')) {
    tags = industryToTagMap['카페'];
  } else if (brandLower.includes('편의점') || brandLower.includes('마트') || brandLower.includes('gs25') || brandLower.includes('cu') || brandLower.includes('세븐일레븐')) {
    tags = industryToTagMap['편의점'];
  } else if (industryLargeCategory === '교육') {
    tags = industryToTagMap['교육'];
  } else if (brandLower.includes('헬스') || brandLower.includes('gym') || brandLower.includes('피트니스') || brandLower.includes('요가')) {
    tags = industryToTagMap['스포츠'];
  } else if (brandLower.includes('뷰티') || brandLower.includes('미용') || brandLower.includes('네일') || brandLower.includes('헤어')) {
    tags = industryToTagMap['뷰티'];
  } else if (brandLower.includes('버거') || brandLower.includes('burger') || brandLower.includes('맥도날드') || brandLower.includes('롯데리아')) {
    tags = industryToTagMap['패스트푸드'];
  } else if (brandLower.includes('베이커리') || brandLower.includes('빵') || brandLower.includes('파리바게뜨')) {
    tags = industryToTagMap['베이커리'];
  } else if (brandLower.includes('아이스크림') || brandLower.includes('빙수') || brandLower.includes('디저트')) {
...
async function processAllBrands() {
  ...
  for (let offset = 0; offset < totalBrands; offset += batchSize) {
    ...
    console.log(`📦 배치 처리 시작 (${offset + 1}~${Math.min(offset + batchSize, totalBrands)})`);
    ...
    for (const brand of brands) {
      try {
        ...
        if (processed % 10 === 0) {
          console.log(`  ✓ ${processed}/${totalBrands} 처리 완료 (${((processed / totalBrands) * 100).toFixed(1)}%)`);
        }
    ...
    console.log(`✅ 배치 처리 완료 (${Math.min(offset + batchSize, totalBrands)}개)`);
    console.log(`📈 전체 진행률: ${((processed / totalBrands) * 100).toFixed(1)}%`);
