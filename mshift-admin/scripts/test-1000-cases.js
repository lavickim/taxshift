...
const { classifyTransaction, KEYWORD_PATTERNS } = require('./test-transaction-classification.js');
...
const EXTENDED_KEYWORD_PATTERNS = {
  ...
  '편의점': {
    keywords: [
      ...
      '7ELV', '7일레븐', '시유', 'BGF리테일', 'CU편의점', 'CU점',
      'GS이십오', 'GS25편의점', 'GS25점', 'GS-25',
      'EMART24', 'E마트24', '이마트이십사', '이마트편의점',
      'MINISTOP', '미니스톱편의점'
    ],
    tag: '편의점',
    accountCode: '622',
  ...
  '음식점': {
    keywords: [
      ...
      '맥날', 'MCD', 'LOTTERIA', '롯데버거', 'BURGERKING', 'BK',
      '켄터키', '케이에프씨', 'SUBWAY', '피자헛', 'PIZZAHUT', '피자',
      '도미노피자', 'DOMINOS', '도미노', '파파존스', '미스터피자',
      '중국집', '홍콩반점', '차이나타운', '김밥천국', '한솥도시락'
    ],
    tag: '음식점',
    accountCode: '651',
    accountName: '접대비',
  ...
  '카페': {
    keywords: [
      ...
      '스벅', 'A-TWOSOME', '투썸', 'EDIYA', '커피빈', 'COFFEEBEAN',
      '할리스커피', 'HOLLYS', '파스쿠찌', 'PASCUCCI', '엔젤리너스', 'ANGELINUS',
      '빽다방', 'PAIK', '공차', 'GONG CHA', '카페'
    ],
    tag: '카페',
    accountCode: '651',
    accountName: '접대비',
  ...
  '온라인쇼핑': {
    keywords: [
      ...
      '일일번가', 'GMARKET', 'AUCTION', '티몬', 'TMON', 'TICKET MONSTER',
      '위메프', 'WEMAKEPRICE', 'WMP', '네이버쇼핑', '카카오쇼핑'
    ],
    tag: '온라인쇼핑',
    accountCode: '634',
    accountName: '소모품비',
  ...
  '교통': {
    keywords: [
      ...
      '전철', '도시철도', '시내버스', '시외버스', '카카오택시', '우버',
      '고속도로', '통행료', '교통카드', 'T머니', '캐시비'
    ],
    tag: '교통',
    accountCode: '611',
    accountName: '여비교통비',
...
function runMassiveTest() {
  ...
  console.log(`성공: ${successCount}개 (${Math.round(successCount / testCases.length * 100)}%)`);
  ...
  const outputPath = path.join(__dirname, 'test-results', 'massive-test-results.json');
...
function generateRecommendations(unmatchedCases, categoryStats) {
  ...
  // 매칭률이 낮으면 키워드 확장 추천
  const totalMatched = Object.values(categoryStats).reduce((sum, count) => sum + count, 0);
  const successRate = totalMatched / (totalMatched + unmatchedCases.length) * 100;
  
  if (successRate < 85) {
    recommendations.push('전체 매칭률이 85% 미만입니다. 키워드 패턴 확장이 필요합니다.');
  }
  
  // 자주 나타나는 미매칭 패턴 분석
  const commonUnmatched = findCommonPatterns(unmatchedCases);
  if (commonUnmatched.length > 0) {
    recommendations.push(`미매칭 케이스에서 자주 나타나는 패턴: ${commonUnmatched.slice(0, 5).join(', ')}`);
  }
  ...
  if (categoryEntries.length > 0) {
    ...
    recommendations.push(`가장 많이 매칭된 카테고리는 '${topCategory[0]}' (${topCategory[1]}건)입니다.`);
  }
  
  if (categoryEntries.length > 1) {
    const bottomCategory = categoryEntries[categoryEntries.length - 1];
    if (bottomCategory[1] < 10) {
      recommendations.push(`'${bottomCategory[0]}' 카테고리의 매칭이 적습니다 (${bottomCategory[1]}건). 키워드 보강을 고려하세요.`);
    }
  }
