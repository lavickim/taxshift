...
function generateKoreanCompanyTransaction(baseName) {
  const companyType =
    KOREAN_COMPANY_SUFFIXES[
      Math.floor(Math.random() * KOREAN_COMPANY_SUFFIXES.length)
    ];
  const paymentMethod =
    PAYMENT_PATTERNS[Math.floor(Math.random() * PAYMENT_PATTERNS.length)];
  const timePattern =
    TIME_PATTERNS[Math.floor(Math.random() * TIME_PATTERNS.length)];
...
function generateVariousKoreanTransactions(count = 100) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (randomType < 0.4) {
      // 40% 대기업 + 회사형태
      const company =
        KOREAN_COMPANIES[Math.floor(Math.random() * KOREAN_COMPANIES.length)];
      transaction = generateKoreanCompanyTransaction(company);
    } else if (randomType < 0.7) {
      // 30% 프랜차이즈 + 회사형태
      const franchise =
        FRANCHISE_VARIATIONS[
          Math.floor(Math.random() * FRANCHISE_VARIATIONS.length)
        ];
    ...
    transactions.push({
      ...
      expectedHasCompanyPattern: transaction.match(
        /(주식회사|㈜|\(주\)|주\/상|주\/하|유한회사|\(유\)|㈲|유\/상|유\/하|합자회사|\(합\)|영업소|지점|지사|대리점|매장|직영점)/
      ),
      category:
        randomType < 0.4
          ? 'large_company'
...
async function updateBrandsWithCompanyPatterns() {
  ...
  try {
    ...
    for (const brand of brands) {
      // 30% 확률로 회사형태 패턴 적용
      if (Math.random() < 0.3) {
        const newTransactionString = generateFranchiseCompanyTransaction(
          brand.brandName
        );
...
async function generateTestTransactions() {
  ...
  testTransactions.slice(0, 10).forEach((t, index) => {
    ...
    console.log(
      `${index + 1}. ${hasPattern} [${t.category}] ${t.transactionString}`
    );
  });

  // 통계
  const companyPatternCount = testTransactions.filter(
    t => t.expectedHasCompanyPattern
  ).length;
  ...
  console.log(
    `   회사형태 패턴 포함: ${companyPatternCount}개 (${((companyPatternCount / testTransactions.length) * 100).toFixed(1)}%)`
  );
