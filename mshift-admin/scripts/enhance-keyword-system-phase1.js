...
const PHASE1_FOOD_TAGS = [
  {
    ...
    tag_name: '한식당개선',
    tag_description:
      '한국 전통 음식 전문점 - 국밥(89% 성공률), 삼겹살, 감자탕(75%), 족발(70%), 곱창(78%), 돼지고기, 돈까스(77%), 칼국수(78%), 덮밥(71%), 닭갈비, 닭고기 포함',
    category: '음식점',
    priority: 1,
  ...
  },
  {
    ...
    tag_name: '치킨전문점개선',
    tag_description:
      '치킨 및 닭요리 전문점 - 치킨(80% 성공률), 닭발(42%), 닭갈비(59%), 닭고기(79%) 등 닭 요리 전문',
    category: '음식점',
    priority: 2,
    expected_success_rate: 0.75,
  },
  {
    ...
    tag_name: '피자전문점개선',
    tag_description:
      '피자 전문점 - 피자(91% 성공률)로 가장 높은 분류 정확도를 보이는 카테고리',
    category: '음식점',
    priority: 3,
  ...
  },
  {
    ...
    tag_name: '중식당개선',
    tag_description:
      '중국 음식 전문점 - 마라탕(69% 성공률), 쌀국수(43%), 짜장면, 짬뽕, 탕수육 등 중국식 요리',
    category: '음식점',
    priority: 4,
  ...
  },
  {
    ...
    tag_name: '일식당개선',
    tag_description:
      '일본 음식 전편점 - 라멘, 우동, 초밥, 돈까스, 일식 정식 등 (현재 48% 성공률로 개선 필요)',
    category: '음식점',
    priority: 5,
  ...
  },
  {
    ...
    tag_name: '양식당개선',
    tag_description:
      '서양 음식 전문점 - 파스타(53% 성공률), 스테이크, 리조또, 피자 이외의 서양식 요리 (현재 48% 성공률)',
    category: '음식점',
    priority: 6,
  ...
  },
  {
    ...
    tag_name: '분식점개선',
    tag_description:
      '분식 전문점 - 떡볶이(79% 성공률), 김밥(85%), 순대, 튀김, 오뎅 등 간단한 한국식 음식',
    category: '음식점',
    priority: 7,
  ...
  },
  {
    ...
    tag_name: '제과점개선',
    tag_description:
      '제과제빵 전문점 - 빵, 케이크, 디저트, 베이커리 제품 전문 (현재 56% 성공률)',
    category: '음식점',
    priority: 8,
  ...
  },
  {
    ...
    tag_name: '주점개선',
    tag_description:
      '주류 제공 음식점 - 술집, 호프집, 포차, 맥주집, 이자카야 등 (현재 52% 성공률)',
    category: '음식점',
    priority: 9,
  ...
  },
  {
    ...
    tag_name: '음식점기타개선',
    tag_description:
      '기타 분류되지 않은 음식점 - 퓨전 요리, 특수 요리, 신규 카테고리 등',
    category: '음식점',
    priority: 10,
...
async function executePhase1Enhancement() {
  ...
  console.log(
    '📋 목표 1: 기존 이상한 태그들 정리 (심야구매, 주말구매, 고액결제)'
  );
  ...
  try {
    await prisma.$transaction(async tx => {
      ...
      for (const mapping of PHASE1_KEYWORD_TAG_MAPPINGS) {
        ...
        `;
        console.log(
          `   ✓ 그룹 ${mapping.keyword_group_id} → 태그 ${mapping.tag_id} 매핑 추가`
        );
      }
      ...
      for (const mapping of PHASE1_TAG_ACCOUNT_MAPPINGS) {
        ...
        `;
        console.log(
          `   ✓ 태그 ${mapping.tag_id} → ${mapping.account_code} ${mapping.account_name} 매핑 추가`
        );
      }
    });
    ...
    console.log(
      `   - 키워드-태그 매핑: ${PHASE1_KEYWORD_TAG_MAPPINGS.length}개`
    );
    console.log(
      `   - 태그-계정과목 매핑: ${PHASE1_TAG_ACCOUNT_MAPPINGS.length}개`
    );
  } catch (error) {
...
async function validatePhase1() {
  ...
  const tagCount =
    await prisma.$queryRaw`SELECT COUNT(*) as count FROM tags_master WHERE id >= 11 AND id <= 20`;
  const groupCount =
    await prisma.$queryRaw`SELECT COUNT(*) as count FROM keyword_groups WHERE id >= 21 AND id <= 30`;
  const mappingCount =
    await prisma.$queryRaw`SELECT COUNT(*) as count FROM keyword_tag_mappings WHERE keyword_group_id >= 21`;
  const accountCount =
    await prisma.$queryRaw`SELECT COUNT(*) as count FROM tag_account_mappings WHERE tag_id >= 11`;
