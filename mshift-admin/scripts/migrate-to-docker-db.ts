  ...
  try {
    ...
    if (gyeonggiData.rows.length > 0) {
      ...
      for (let i = 0; i < gyeonggiData.rows.length; i += 1000) {
        ...
        console.log(
          `📦 경기도 배달특급 데이터 ${i + batch.length}/${gyeonggiData.rows.length} 완료`
        );
      }
    }
    ...
    const seoulData = await sourceClient.query(
      'SELECT * FROM datacollection_seoul_restaurants ORDER BY created_at LIMIT 10000'
    );
    console.log(
      `🍽️ 서울시 음식점 데이터: ${seoulData.rows.length}개 (처음 10,000개만)`
    );
    ...
    if (seoulData.rows.length > 0) {
      ...
      for (let i = 0; i < seoulData.rows.length; i += 500) {
        ...
        console.log(
          `🍽️ 서울시 음식점 데이터 ${i + batch.length}/${seoulData.rows.length} 완료`
        );
      }
    }
