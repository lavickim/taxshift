...
class TestSuite {
  ...
  async run() {
    ...
    log('info', `📊 ${this.name} 완료: ${suiteResult.passed}개 통과, ${suiteResult.failed}개 실패`);
...
// 1. 시스템 헬스 체크 테스트
const healthCheckSuite = new TestSuite('시스템 헬스 체크', '전체 시스템 구성요소의 상태를 확인합니다.');

healthCheckSuite
  .test('프론트엔드 서버 응답', async () => {
    const result = await makeRequest(`${TEST_CONFIG.baseUrl}/api/v2/system/health`);
    expect(result.success).toBeTruthy();
    expect(result.data.overall).toContain('healthy');
    return result.data;
  })
  .test('백엔드 API 연결', async () => {
    const result = await makeRequest(`${TEST_CONFIG.javaApiUrl}/actuator/health`);
    // 백엔드가 없어도 테스트를 계속하기 위해 조건부 체크
    if (result.success) {
      expect(result.data.status).toBe('UP');
    } else {
      log('warn', '백엔드 서버에 연결할 수 없습니다. 목 데이터로 테스트를 계속합니다.');
    }
    return result;
  });

// 2. 키워드 그룹 관리 테스트
const keywordGroupSuite = new TestSuite('키워드 그룹 관리', '키워드 그룹 CRUD 작업을 테스트합니다.');

let testGroupId = null;

keywordGroupSuite
  .test('키워드 그룹 목록 조회', async () => {
    const result = await makeRequest(`${TEST_CONFIG.baseUrl}/api/v2/tag-mapping/keyword-groups`);
    expect(result.success).toBeTruthy();
  ...
  .test('키워드 그룹 수정', async () => {
    ...
    const result = await makeRequest(`${TEST_CONFIG.baseUrl}/api/v2/tag-mapping/keyword-groups/${testGroupId}`, {
      method: 'PUT',
...
// 3. 태그 추천 엔진 테스트
const recommendationSuite = new TestSuite('태그 추천 엔진', '키워드 추출 및 태그 추천 기능을 테스트합니다.');
...
recommendationSuite
  .test('편의점 키워드 추천', async () => {
    ...
    for (const testCase of testCases) {
      const result = await makeRequest(`${TEST_CONFIG.baseUrl}/api/v2/tag-mapping/recommend-tags`, {
        method: 'POST',
        body: JSON.stringify({
      ...
      if (result.data.length > 0) {
        ...
        expect(recommendation.tagMapping.tag.tagName).toBe(testCase.expectedTag);
...
// 4. 성능 테스트
const performanceSuite = new TestSuite('성능 테스트', 'API 응답 시간 및 처리량을 테스트합니다.');
...
performanceSuite
  ...
  .test('동시 요청 처리', async () => {
    ...
    for (let i = 0; i < testCount; i++) {
      promises.push(
        makeRequest(`${TEST_CONFIG.baseUrl}/api/v2/tag-mapping/recommend-tags`, {
          method: 'POST',
          body: JSON.stringify({
            keyword: `테스트${i}`,
...
// 5. 오류 처리 테스트
const errorHandlingSuite = new TestSuite('오류 처리', '잘못된 입력 및 오류 상황을 테스트합니다.');
...
errorHandlingSuite
  ...
  .test('존재하지 않는 그룹 수정', async () => {
    const result = await makeRequest(`${TEST_CONFIG.baseUrl}/api/v2/tag-mapping/keyword-groups/99999`, {
      method: 'PUT',
      body: JSON.stringify({
        groupName: '존재하지않음',
        primaryKeyword: '테스트',
...
async function runAllTests() {
  ...
  // 종료 코드 설정
  const exitCode = testResults.summary.failed > 0 || testResults.summary.errors.length > 0 ? 1 : 0;
