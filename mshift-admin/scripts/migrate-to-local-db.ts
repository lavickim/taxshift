...
async function migrateData() {
  try {
    ...
    console.log(`📦 경기도 배달특급 데이터: ${(gyeonggiDeliveryData as any[]).length}개`)
    ...
    console.log(`🍽️ 서울시 음식점 데이터: ${(seoulRestaurantsData as any[]).length}개`)
    ...
    const transactionCache = await supabaseClient.$queryRaw`SELECT * FROM transaction_cache LIMIT 100`
    console.log(`💾 Transaction Cache: ${(transactionCache as any[]).length}개`)

    console.log('\n📋 마이그레이션할 데이터 요약:')
    console.log(`- 경기도 배달특급: ${(gyeonggiDeliveryData as any[]).length}개`)
