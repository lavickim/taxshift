...
async function checkPriorities() {
  ...
  try {
    ...
    paymentGroups.forEach(group => {
      console.log(`  ${group.id}. ${group.groupName} (${group.primaryKeyword}) - ${group.category}`);
      console.log(`     동의어: [${group.synonyms.slice(0, 5).join(', ')}${group.synonyms.length > 5 ? '...' : ''}]`);
    });
    ...
    mappingPriorities.forEach(mapping => {
      console.log(`  매핑 ${mapping.id}: 그룹${mapping.keywordGroupId} → 태그${mapping.tagId} (우선순위: ${mapping.priority || 'NULL'}, 신뢰도: ${mapping.confidenceScore})`);
    });
