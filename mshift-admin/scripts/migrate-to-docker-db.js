...
async function migrateData() {
  try {
    ...
    const gyeonggiData = await sourceClient.query(
      'SELECT * FROM datacollection_gyeonggi_delivery ORDER BY created_at'
    );
    console.log(`📦 경기도 배달특급 데이터: ${gyeonggiData.rows.length}개`);

    if (gyeonggiData.rows.length > 0) {
      // 하나씩 삽입 (안전하게)
      for (let i = 0; i < gyeonggiData.rows.length; i++) {
        const row = gyeonggiData.rows[i];
        try {
          await targetClient.query(
            `
          ...
          `,
            [
              ...
              row.store_name,
              row.industry_type,
              row.refined_road_address,
              row.refined_lot_address,
              row.refined_zipcode,
              row.refined_latitude,
              row.refined_longitude,
              row.data_source,
              row.collection_batch_id,
              row.created_at,
              row.updated_at,
            ]
          );
          ...
          if (i % 1000 === 0) {
            console.log(
              `📦 경기도 배달특급 데이터 ${i + 1}/${gyeonggiData.rows.length} 완료`
            );
          }
        } catch (error) {
          console.log(
            `⚠️ 경기도 데이터 삽입 오류 (계속 진행): ${error.message}`
          );
        }
      }
    }

    // 3. 서울시 음식점 데이터 마이그레이션 (처음 5000개만)
    console.log('🏪 서울시 음식점 데이터 마이그레이션 중...');

    const seoulData = await sourceClient.query(
      'SELECT * FROM datacollection_seoul_restaurants ORDER BY created_at LIMIT 5000'
    );
    console.log(
      `🍽️ 서울시 음식점 데이터: ${seoulData.rows.length}개 (처음 5,000개만)`
    );

    if (seoulData.rows.length > 0) {
      for (let i = 0; i < seoulData.rows.length; i++) {
        const row = seoulData.rows[i];
        try {
          await targetClient.query(
            `
          ...
          `,
            [
              ...
              row.license_date,
              row.license_cancel_date,
              row.business_status_code,
              row.business_status_name,
              row.detailed_status_code,
              row.detailed_status_name,
              row.closure_date,
              row.suspension_start_date,
              row.suspension_end_date,
              row.reopening_date,
              row.phone_number,
              row.site_area,
              row.postal_code,
              row.lot_address,
              row.road_address,
              row.road_postal_code,
              row.business_name,
              row.last_modified_date,
              row.data_update_type,
              row.data_update_date,
              row.business_type,
              row.coordinate_x,
              row.coordinate_y,
              row.hygiene_business_type,
              row.male_employee_count,
              row.female_employee_count,
              row.surrounding_area_type,
              row.grade_type,
              row.water_supply_type,
              row.total_employees,
              row.headquarters_employees,
              row.factory_office_employees,
              row.factory_sales_employees,
              row.factory_production_employees,
              row.building_ownership_type,
              row.deposit_amount,
              row.monthly_rent,
              row.multi_use_facility_yn,
              row.total_facility_size,
              row.traditional_business_number,
              row.traditional_main_food,
              row.homepage,
              row.data_source,
              row.collection_batch_id,
              row.created_at,
          ...
          if (i % 500 === 0) {
            console.log(
              `🍽️ 서울시 음식점 데이터 ${i + 1}/${seoulData.rows.length} 완료`
            );
          }
        } catch (error) {
          console.log(`⚠️ 서울 데이터 삽입 오류 (계속 진행): ${error.message}`);
        }
      }
    }
    ...
    const gyeonggiCount = await targetClient.query(
      'SELECT COUNT(*) FROM datacollection_gyeonggi_delivery'
    );
    const seoulCount = await targetClient.query(
      'SELECT COUNT(*) FROM datacollection_seoul_restaurants'
    );
