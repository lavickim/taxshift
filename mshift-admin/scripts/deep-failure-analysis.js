...
async function deepFailureAnalysis() {
  ...
  try {
    ...
    for (let i = 0; i < sampleBrands.length; i += batchSize) {
      ...
      const currentAccuracy = ((totalMatched / results.length) * 100).toFixed(
        2
      );
      console.log(
        `   📈 현재까지 정확도: ${currentAccuracy}% (${totalMatched}/${results.length})`
      );
    ...
    const finalAccuracy = (
      (successCases.length / results.length) *
      100
    ).toFixed(2);
    console.log(`\n📊 최종 분석 완료`);
    ...
    for (const failed of failedCases) {
      if (failed.error) {
        ...
        !failed.extractedKeywords ||
        failed.extractedKeywords.length === 0
      ) {
    ...
    console.log(`\n📈 실패 유형별 분포:`);
    console.log(
      `   키워드 추출 실패: ${failureTypes.NO_KEYWORDS_EXTRACTED.length}개 (${((failureTypes.NO_KEYWORDS_EXTRACTED.length / failedCases.length) * 100).toFixed(1)}%)`
    );
    console.log(
      `   키워드는 있으나 매칭 실패: ${failureTypes.KEYWORDS_NOT_MATCHED.length}개 (${((failureTypes.KEYWORDS_NOT_MATCHED.length / failedCases.length) * 100).toFixed(1)}%)`
    );
    console.log(
      `   잘못된 태그 매핑: ${failureTypes.WRONG_TAG_MAPPED.length}개 (${((failureTypes.WRONG_TAG_MAPPED.length / failedCases.length) * 100).toFixed(1)}%)`
    );
    console.log(
      `   API 오류: ${failureTypes.API_ERROR.length}개 (${((failureTypes.API_ERROR.length / failedCases.length) * 100).toFixed(1)}%)`
    );
    ...
    failureTypes.NO_KEYWORDS_EXTRACTED.slice(0, 30).forEach((failed, index) => {
      console.log(
        `${index + 1}. ${failed.brandName} (${failed.industryLargeCategory})`
      );
    ...
    for (const failed of failedCases) {
      ...
      if (/[^\w\s가-힣]/.test(brandName)) {
        brandPatterns['특수문자포함'] =
          (brandPatterns['특수문자포함'] || 0) + 1;
      }
      ...
      if (/역|점|센터|타운|몰|마트/.test(brandName)) {
        brandPatterns['지역/매장명포함'] =
          (brandPatterns['지역/매장명포함'] || 0) + 1;
      }
    }
    ...
    Object.entries(brandPatterns)
      ...
      .forEach(([pattern, count]) => {
        console.log(
          `   ${pattern}: ${count}개 (${((count / failedCases.length) * 100).toFixed(1)}%)`
        );
      });
    ...
    Object.entries(industryFailures)
      ...
      .forEach(([industry, count]) => {
        const industryTotal = results.filter(
          r => r.industryLargeCategory === industry
        ).length;
      ...
      if (failed.mainProduct) {
        ...
        for (const product of products) {
          ...
          if (cleanProduct.length > 1) {
            productFailures[cleanProduct] =
              (productFailures[cleanProduct] || 0) + 1;
          }
        }
      }
    }
    ...
    const successKeywordCount =
      successCases.reduce((sum, s) => sum + s.extractedKeywords.length, 0) /
      successCases.length;
    const failureKeywordCount =
      failedCases
        .filter(f => f.extractedKeywords.length > 0)
        .reduce((sum, f) => sum + f.extractedKeywords.length, 0) /
      failedCases.filter(f => f.extractedKeywords.length > 0).length;

    console.log(
      `   성공 케이스 평균 키워드 수: ${successKeywordCount.toFixed(2)}개`
    );
    console.log(
      `   실패 케이스 평균 키워드 수: ${failureKeywordCount.toFixed(2)}개`
    );

    const successConfidence =
      successCases.reduce((sum, s) => sum + s.confidence, 0) /
      successCases.length;
...
function generateRecommendations(
  ...
  if (failureTypes.KEYWORDS_NOT_MATCHED.length > 0) {
    recommendations.push({
      ...
      solution:
        '추출된 키워드들에 대한 동의어 사전 확장 및 새로운 키워드 그룹 생성',
  ...
  // 산업별 해결방안
  const topFailureIndustry = Object.entries(industryFailures).sort(
    (a, b) => b[1] - a[1]
  )[0];
