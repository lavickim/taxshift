...
const KEYWORD_TAG_MAPPINGS = [
  // 냉면 전문점
  { keywordPattern: '냉면', tagId: 70, confidenceScore: 0.90, priority: 1 },
  ...
  { keywordPattern: '뿅가야채곱창', tagId: 72, confidenceScore: 0.95, priority: 1 },
  { keywordPattern: '뿔난황소곱창', tagId: 72, confidenceScore: 0.95, priority: 1 },
  ...
  { keywordPattern: '허갈닭강정', tagId: 74, confidenceScore: 0.95, priority: 1 },
  { keywordPattern: '부자치킨', tagId: 74, confidenceScore: 0.95, priority: 1 },
  
  // 커피 전문점
  { keywordPattern: '커피', tagId: 75, confidenceScore: 0.90, priority: 1 },
  { keywordPattern: '빅피처커피', tagId: 75, confidenceScore: 0.95, priority: 1 },
  
  // 일식 전문점
  { keywordPattern: '일식', tagId: 76, confidenceScore: 0.80, priority: 1 },
  ...
  { keywordPattern: 'After Glow', tagId: 77, confidenceScore: 0.95, priority: 1 }
];

// 태그-계정과목 매핑 정의
const TAG_ACCOUNT_MAPPINGS = [
  // 음식점 태그들 - 기본 접대비
  { tagId: 70, accountCode: '651', accountName: '접대비', isDefault: true, priority: 1 },
  { tagId: 71, accountCode: '651', accountName: '접대비', isDefault: true, priority: 1 },
  { tagId: 72, accountCode: '651', accountName: '접대비', isDefault: true, priority: 1 },
  { tagId: 73, accountCode: '651', accountName: '접대비', isDefault: true, priority: 1 },
  { tagId: 74, accountCode: '651', accountName: '접대비', isDefault: true, priority: 1 },
  { tagId: 75, accountCode: '651', accountName: '접대비', isDefault: true, priority: 1 },
  { tagId: 76, accountCode: '651', accountName: '접대비', isDefault: true, priority: 1 },
  
  // 이미용 서비스 - 사무용품비
  { tagId: 77, accountCode: '634', accountName: '사무용품비', isDefault: true, priority: 1 },
  
  // 조건부 매핑들 - 야근식대 (심야시간대)
  { 
    tagId: 70, accountCode: '655', accountName: '야근식대', isDefault: false, priority: 2,
    mappingCondition: JSON.stringify({ timeRange: '22:00-06:00', amountMax: 30000 })
  },
  { 
    tagId: 71, accountCode: '655', accountName: '야근식대', isDefault: false, priority: 2,
    mappingCondition: JSON.stringify({ timeRange: '22:00-06:00', amountMax: 50000 })
  },
  { 
    tagId: 74, accountCode: '655', accountName: '야근식대', isDefault: false, priority: 2,
    mappingCondition: JSON.stringify({ timeRange: '22:00-06:00', amountMax: 40000 })
  },
  { 
    tagId: 75, accountCode: '655', accountName: '야근식대', isDefault: false, priority: 3,
    mappingCondition: JSON.stringify({ timeRange: '22:00-06:00', amountMax: 15000 })
  },
  
  // 조건부 매핑들 - 복리후생비 (주말 고액)
  { 
    tagId: 71, accountCode: '653', accountName: '복리후생비', isDefault: false, priority: 3,
    mappingCondition: JSON.stringify({ dayOfWeek: 'weekend', amountMin: 80000 })
  },
  { 
    tagId: 73, accountCode: '653', accountName: '복리후생비', isDefault: false, priority: 3,
    mappingCondition: JSON.stringify({ dayOfWeek: 'weekend', amountMin: 60000 })
  }
];
...
async function createKeywordTagMappings() {
  ...
  for (const mapping of KEYWORD_TAG_MAPPINGS) {
    // 매핑에 해당하는 키워드 그룹 찾기
    const matchingGroups = keywordGroups.filter(group => 
      group.primaryKeyword.includes(mapping.keywordPattern) ||
      group.synonyms.some(synonym => synonym.includes(mapping.keywordPattern)) ||
    ...
    for (const group of matchingGroups) {
      try {
        ...
        console.log(`   ✓ ${group.groupName} → 태그 ${mapping.tagId} 매핑 완료`);
        successCount++;
      } catch (error) {
        if (error.code === 'P2002') {
          console.log(`   ⚠️ ${group.groupName} → 태그 ${mapping.tagId} 이미 존재`);
        } else {
          console.log(`   ❌ ${group.groupName} → 태그 ${mapping.tagId} 실패: ${error.message}`);
        }
      }
    }
  }
...
async function createTagAccountMappings() {
  ...
  for (const mapping of TAG_ACCOUNT_MAPPINGS) {
    try {
      ...
      console.log(`   ✓ 태그 ${mapping.tagId} → ${mapping.accountCode} ${mapping.accountName}${conditionText}`);
      successCount++;
    } catch (error) {
      if (error.code === 'P2002') {
        console.log(`   ⚠️ 태그 ${mapping.tagId} → ${mapping.accountCode} 이미 존재`);
      } else {
        console.log(`   ❌ 태그 ${mapping.tagId} → ${mapping.accountCode} 실패: ${error.message}`);
      }
    }
  }
...
async function executeCompleteTagAccountMappings() {
  ...
  try {
    ...
    if (cacheRefreshed) {
      ...
      console.log('🧪 이제 브랜드 테스트에서 태그와 계정과목이 정상적으로 나올 것입니다.');
    }
