...
async function seedFinalPatterns() {
  ...
  console.log(
    `📊 총 ${Object.keys(FINAL_75_KEYWORD_PATTERNS).length}개 카테고리 패턴 시딩\n`
  );
  ...
  try {
    ...
    // 각 카테고리별로 키워드 그룹 생성
    for (const [categoryKey, pattern] of Object.entries(
      FINAL_75_KEYWORD_PATTERNS
    )) {
      ...
      console.log(
        `   ✅ ${pattern.keywords.length}개 키워드, 계정코드: ${pattern.accountCode}`
      );
    }
    ...
    sampleGroups.forEach((group, index) => {
      ...
      console.log(
        `   계정코드: ${mapping?.accountCode || 'N/A'} - ${mapping?.accountName || 'N/A'}`
      );
      console.log(
        `   신뢰도: ${Math.round((group.confidenceBase || 0) * 100)}%`
      );
      console.log(
        `   샘플 키워드: ${group.synonyms.slice(0, 3).join(', ')}...\n`
      );
    });
