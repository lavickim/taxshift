...
async function updateTagAccountMappings() {
  ...
  try {
    ...
    for (const accountCode of targetAccountCodes) {
      ...
      if (!account) {
        throw new Error(
          `대상 계정 ${accountCode}가 존재하지 않습니다. 먼저 확장 계정과목을 생성해주세요.`
        );
      }
    }
    ...
    for (const mapping of oldCodeMappings) {
      ...
      const newCode =
        accountCodeMapping[oldCode as keyof typeof accountCodeMapping];
      ...
      if (newCode) {
        ...
        updatedCount++;
        console.log(
          `  ✓ ${mapping.tag.tagName}: ${oldCode}(${mapping.accountName}) → ${newCode}(${newAccount?.accountName})`
        );
      }
    }
    ...
    for (const [tagName, recommendedAccountCode] of Object.entries(
      businessTagMappings
    )) {
      ...
      if (tag) {
        ...
        if (
          existingMapping &&
          existingMapping.accountCode !== recommendedAccountCode
        ) {
          ...
          if (recommendedAccount) {
            ...
            optimizedCount++;
            console.log(
              `  🎯 ${tagName}: ${existingMapping.accountCode} → ${recommendedAccountCode}(${recommendedAccount.accountName})`
            );
          }
        }
      }
    ...
    if (finalMappings.length === 0) {
      console.log(
        '✅ 모든 3자리 계정코드가 성공적으로 4자리로 변환되었습니다!'
      );
    } else {
      console.log(
        `⚠️ 아직 ${finalMappings.length}개의 3자리 코드가 남아있습니다.`
      );
    }

    // 8. 업데이트된 매핑 통계
    const updatedStats = await Promise.all([
      prisma.tagAccountMapping.count({
        where: { accountCode: { startsWith: '5' } },
      }), // 비용 계정
    ...
    console.log(
      '💡 다음 단계: yarn test __tests__/accounting/tag-account-mapping-update.test.ts'
    );
  } catch (error) {
...
if (require.main === module) {
  updateTagAccountMappings().catch(error => {
    console.error(error);
    process.exit(1);
  });
}
