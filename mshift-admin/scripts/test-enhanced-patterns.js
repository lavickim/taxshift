...
const { classifyTransactionEnhanced } = require('./enhanced-keyword-patterns.js');
...
function runEnhancedTest() {
  ...
  // 이전 결과와 비교
  try {
    const previousResults = JSON.parse(fs.readFileSync('./test-results/massive-test-results.json', 'utf8'));
  ...
  if (successRate < 85) {
    enhancedResults.recommendations.push('매칭률이 85% 미만입니다. 추가 키워드 패턴 확장이 필요합니다.');
  }
  ...
  if (unmatchedCases.length > 100) {
    ...
    if (commonWords.length > 0) {
      enhancedResults.recommendations.push(`자주 나타나는 미매칭 패턴: ${commonWords.join(', ')}`);
    }
  }
  ...
  fs.writeFileSync(path.join(outputDir, 'enhanced-test-results.json'), JSON.stringify(enhancedResults, null, 2));
