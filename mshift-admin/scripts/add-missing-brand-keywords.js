...
async function addMissingBrandKeywords() {
  ...
  try {
    ...
    for (const rule of missingBrandKeywords) {
      try {
        ...
        if (!existingMapping) {
          ...
          addedMappings++;
          console.log(
            `  ✅ 키워드-태그 매핑 생성 (ID: ${keywordTagMapping.id}, 우선순위: 200)`
          );
        } else {
    ...
    priorityDistribution.forEach(dist => {
      console.log(
        `   우선순위 ${dist.priority || 'NULL'}: ${dist._count.id}개`
      );
    });
    ...
    try {
      const response = await fetch(
        'http://localhost:8080/v2/tag-mapping/refresh-cache',
        {
          method: 'POST',
        }
      );
