...
async function testDynamicBrandSystem() {
  ...
  try {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            `   🔑 키워드: [${result.extractedKeywords?.join(', ') || ''}]`
          );
    ...
    console.log(
      `키워드 엔진 매칭: ${keywordEngineMatches}개 (${((keywordEngineMatches / testCases.length) * 100).toFixed(1)}%)`
    );
    console.log(
      `동적 브랜드 매칭: ${dynamicBrandMatches}개 (${((dynamicBrandMatches / testCases.length) * 100).toFixed(1)}%)`
    );
    console.log(
      `매칭 실패: ${noMatches}개 (${((noMatches / testCases.length) * 100).toFixed(1)}%)`
    );
    console.log(
      `전체 매칭률: ${(((keywordEngineMatches + dynamicBrandMatches) / testCases.length) * 100).toFixed(1)}%`
    );
    ...
    // 동적 브랜드 매칭 사례
    const dynamicMatches = results.filter(
      r => r.processingPath === 'DYNAMIC_BRAND'
    );
    ...
    if (dynamicBrandMatches > 0) {
      console.log(
        `✅ 동적 브랜드 검색으로 ${dynamicBrandMatches}개 추가 매칭 성공`
      );
      console.log(
        `✅ 기존 키워드 시스템 대비 ${dynamicBrandMatches}개 케이스 개선`
      );
    ...
    return {
      ...
      noMatches,
      overallMatchRate:
        ((keywordEngineMatches + dynamicBrandMatches) / testCases.length) * 100,
      results,
    };
  } catch (error) {
