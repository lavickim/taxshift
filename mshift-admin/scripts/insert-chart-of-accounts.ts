...
async function insertChartOfAccounts() {
  ...
  try {
    ...
    if (existingCount > 0) {
      console.log(
        '⚠️  이미 계정과목 데이터가 존재합니다. 덮어쓰시겠습니까? (중복 방지를 위해 스킵)'
      );
      return;
    }
    ...
    accounts.forEach((account: any) => {
      console.log(
        `${account.accountCode} | ${account.accountName.padEnd(12)} | ${account.accountType} - ${account.accountSubtype || ''}`
      );
    });
...
if (require.main === module) {
  insertChartOfAccounts().catch(error => {
    console.error(error);
    process.exit(1);
  });
}
